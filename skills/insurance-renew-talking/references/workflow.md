# Renewal reminder talking-clip workflow

## Slot list

Write one slot per still before any paid call. Default three slots
unless the advisor names another count in 2 to 8: expiry notice,
renewal deadline, and grace-period wording. Each slot names the still,
the spoken line from the supplied renewal schedule, and a planned
2–15s window. Do not invent an expiry date, renewal deadline, grace
period, premium amount, coverage guarantee, or buy recommendation not
on the schedule. Do not force renewal. The labeled list is the free
...
```json
{
  "input": "The expiry date is the one already printed on this renewal schedule.",
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-renew-speech-01"
}
```
...
```json
{
  "image": {"type": "artifact", "artifact_id": "art_renew_01"},
  "driving_audio": {"type": "artifact", "artifact_id": "art_renew_speech_01"},
  "prompt": "A restrained renewal reminder read from this still. Keep face, notice graphic, and lighting stable.",
  "duration": 8,
  "client_request_id": "opaque-renew-video-01"
}
```
...
lines match the supplied renewal schedule. Report only what the host
