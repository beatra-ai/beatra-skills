---
name: "insurance-renew-talking"
description: "Turn user-supplied policy renewal dates and authorized stills into one insurance renewal reminder talking clip per still. This insurance renewal reminder talking video studio writes a speakable expiry-date talking clip for each photo, then animates a 2 to 15s renewal reminder talking clip. Use it for insurance renewal reminder talking packs that stay one photo, one clip."
---

# Insurance Renewal Reminder Talking Clips

Turn user-supplied policy renewal dates and authorized stills into one
talking clip per still. Deliver 2 to 8 clips. Do not stitch them. Speak
only expiry and renewal-window facts already printed in the supplied
schedule. Do not force renewal.

## Scope and adjacent routes

Use this Skill when an insurance advisor or wealth educator wants short
talking clips that remind clients about renewal dates they already have.

Route a generic presenter that is not a renewal read to
`talking-avatar-video`. Route a clause explanation read to
`insurance-clause-talking`. Route a wealth FAQ read to `wealth-faq-talking`.
Route insurance comparison stills with no talking video to
`insurance-compare-set`. Route silent product motion to
`product-video-studio`.

## Collect the renewal schedule

Hard inputs are:

- at least one accessible still the host Agent can inspect — a
  presenter portrait or a renewal notice graphic that will be the first
  frame;
- the policy renewal schedule the advisor supplied (policy name,
  expiry date, renewal deadline, grace period wording, and other
  printed dates they want spoken);
- a pronunciation table whenever product names, insurer names, or coined
  words appear;
...
Reuse already-known language, product names, and a frozen `voice_id`.
Ask only for a missing hard input. A count outside 2 to 8 is still
doable: confirm that pack size and its live cost.

Do not invent an expiry date, renewal deadline, grace period, premium
amount, coverage guarantee, or buy recommendation that is not on the
schedule. Do not force renewal or promise uninterrupted coverage. File
access is not consent.

If product or insurer names exist and the pronunciation table is empty,
stop and collect the readings.
...
Write a labeled renewal reminder talking list before any paid clone,
speech, or video. Default three slots unless the advisor names another
count in 2 to 8: expiry notice, renewal deadline, and grace-period
wording. Each slot records the still, the spoken line from the supplied
schedule, intended length as a 2–15s clip, and whether it uses a catalog
voice, an approved track, or a clone.
...
its own six-field card and its own opaque `client_request_id` per slot.
...
6. If the balance is insufficient — relay the official message and
   ...
   retry until the advisor says they have topped up.
...
1. Work — one spoken renewal reminder line per named slot
...
1. Work — one talking renewal reminder clip per named still
...
Review that each clip stays on the supplied renewal schedule line and
that the still is the first frame. Report only what the host can
actually see and hear. Do not promise perfect lip sync. Never invent a
stitch, concat, or editor tool. After each terminal paid task, deliver
actual bytes plus MIME, duration, and size when present, and
...
arguments. A changed still, line, voice, or duration is a new card and
a new ID. Cancel only when the advisor asks.
...
```text
printf '%s' '{"input":"The expiry date is the one already printed on this renewal schedule.","voice":"voice-ready-01","format":"mp3","speed":1.0,"model":"auto","client_request_id":"opaque-renew-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```

```text
printf '%s' '{"image":{"type":"artifact","artifact_id":"art-renew-01"},"driving_audio":{"type":"artifact","artifact_id":"art-speech-01"},"prompt":"A restrained insurance renewal reminder read with steady eye line and a stable camera.","duration":8,"client_request_id":"opaque-renew-video-01"}' | python3 scripts/mcp_client.py call beatra.videos.animate
```
...
- For slot lists, payloads, and recovery, read
  [Renewal reminder talking workflow](references/workflow.md).
