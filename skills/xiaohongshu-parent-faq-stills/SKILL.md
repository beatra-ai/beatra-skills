---
name: "xiaohongshu-parent-faq-stills"
description: "Turn Xiaohongshu parent FAQ notes into a 4 to 8 still answer set. This parent FAQ still studio reads public parent notes and questions, then lays out an answer still pack from confirmed teaching facts. Use it for parent FAQ graphics, answer stills, a Xiaohongshu parent FAQ still, and an answer card set."
---

# Xiaohongshu Parent FAQ Stills

Turn Xiaohongshu parent FAQ notes into one answer still per
picked parent question. Deliver 4 to 8 stills.

## Scope and adjacent routes

Use this Skill when a school teacher wants answer stills that
reply to public Xiaohongshu parent-FAQ notes from teaching facts
they already confirmed. Do not score a student.

Route a research memo to `xiaohongshu-note-research`. Route hall
materials stills to `xiaohongshu-hall-faq-stills`. Route lesson-caption
knowledge-point stills to `youtube-lesson-card-set` when that package
is installed.

This package reads Xiaohongshu parent-FAQ notes and comments and
makes answer stills. It does not post a note on Xiaohongshu.
...
- the named class or parent-FAQ topic being answered;
- either a public Xiaohongshu note link or search wording the host
  can open, or the original parent questions already copied;
- confirmed teaching facts the cards may print;
- how many cards the pack should contain, or permission to use the
  default of 4.

Reuse already-known class names, brand palette, and destination.
Ask only for a missing hard input. A count outside 4 to 8 is still
doable: confirm that pack size and its live cost.

Do not invent a parent question, username, like count, score, or
answer line. An optional brand still is a look reference, not a
source for missing answers. File access is not consent.
...
Write a labeled question-to-answer list before any paid lookup
or image. Default four slots unless the user names another count in
4 to 8. Each slot records the original parent question wording, the
printed answer line from confirmed facts, layout (question,
answer line, optional footer), canvas, and whether a brand still
drives the look.
...
user wants this connection to read public Xiaohongshu parent-FAQ
notes or comments, follow
[parent FAQ lookup](references/parent-faq-lookup.md): search, inspect,
then show a six-field lookup card and wait:

1. Work — one public Xiaohongshu note search, note read, or comment
   page for the named parent FAQ (`beatra.social.execute` with the
...
1. Work — one answer still per named slot
...
3. Count — one paid generate call per slot. Do not batch several
   parent questions into one `count`.
...
confirmed answer line. Report only the text the host can actually
see. Treat generated small type as a review item, not as a certified
grade. After each terminal paid task, deliver actual bytes
...
byte-identical arguments. A changed parent question, answer line,
...
- For slot lists, lookup, payloads, and recovery, read
  [parent FAQ still workflow](references/workflow.md) and
