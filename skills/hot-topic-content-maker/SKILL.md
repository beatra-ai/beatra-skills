...
8. **Confirm the video** when a clip was included. Then call `beatra.videos.animate` once with the approved opening frame, the narration, an explicitly selected model, and the smallest supported whole-second duration at or above the real narration length within that card's live range.
...
Record each task ID immediately and poll only that task. `queued` and `running` mean wait. If a create response is lost, resubmit only the identical frozen payload under the same identifier; if a task ID is lost, list tasks for that capability and match candidates against your own ledger before any retry — except for a trend lookup, which has no capability to list by and is matched on its saved `operation_key`, arguments, and `schema_hash`. Redoing the cover reuses the clip artifacts unchanged. `insufficient_balance` means nothing was charged and the identical request can be resubmitted after a top-up.
