...
directly instead: the field shows what covers this topic is already publishing against — their wording, their crops, their counts; a pasted reference note returns that note's own cover image; the comments say which promise readers actually reacted to, which is what the cover line has to carry; and the account's own recent notes are where a drifted cover style becomes visible.
