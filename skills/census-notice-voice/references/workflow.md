# Census notice voice workflow

## Slot list

Write one slot per written schedule item before any paid call.
Default ten slots unless the office names another count in 8 to 20:
notice time, documents, visit window, enumerator intro, privacy,
if-not-home, contact, follow-up, wrap-up, and extra. Each slot names
the spoken line from the written census schedule. Omit a default
role whose line is missing. Do not invent a census result, a
penalty, a personal record, or an eligibility finding. The labeled
...
```json
{
  ...
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-census-speech-01"
}
```
...
pronunciation table and the written census schedule. Do not invent a
census result. After a returned `task_id`, poll that task. If the
