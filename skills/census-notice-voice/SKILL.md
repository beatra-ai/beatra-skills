---
name: "census-notice-voice"
description: "Turn a written census schedule into one census notice voice clip per labeled cue. This census schedule voice studio records each census notice audio and arrangement voice from the list the office already wrote, then delivers 8 to 20 census notice voice pack files. Use it for census clip packs that keep one schedule item on each clip."
---

# Census Notice Voice Pack

Turn a written census schedule into one spoken clip per labeled cue.
Deliver 8 to 20 reads. Cover notice time, documents, and visit window
first.

## Scope and adjacent routes

Use this Skill when a street office needs labeled census-notice clips
from the schedule they already wrote.
...
window list to `hall-guide-voice`. Route visitor-desk lines to
`visitor-desk-voice`.

Read only the supplied census schedule. Do not invent a census result,
a penalty, a personal record, or an eligibility finding.

## Collect the schedule

Hard inputs are:

- the census schedule the office already wrote;
...
Do not invent a census result, a penalty, a personal record, or an
eligibility finding. File access is not consent.
...
Write a labeled census-notice voice list before any paid clone or
speech. Default ten slots unless the office names another count in
8 to 20: notice time, documents, visit window, enumerator intro,
privacy, if-not-home, contact, follow-up, wrap-up, and extra. Each
slot records the spoken line from the written schedule and whether it
uses a catalog voice or a clone. Omit a default role whose line is
...
1. Work — one spoken schedule cue per named slot
   (`beatra.speech.synthesize`).
...
pronunciation table and the written census schedule. Report only
what the host can actually hear. Do not invent a census result.
...
```text
printf '%s' '{"input":"<the written line for this slot>","voice":"voice_...","format":"mp3","client_request_id":"opaque-census-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```
