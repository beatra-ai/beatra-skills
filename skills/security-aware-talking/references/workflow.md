# Security-awareness talking-clip workflow

## Slot list

Write one slot per still before any paid call. Default three slots
unless the team names another count in 2 to 8: password hygiene,
phishing recognition, and device locking — only when those points
already appear in the supplied brief. Each slot names the still, the
spoken line from the supplied talking points, and a planned 2–15s
window. Do not invent company policy, run a live phishing drill, or
produce phishing-drill posters. The labeled list is the free artifact.
It is not approval.
...
```json
{
  "input": "<the written awareness line for this slot>",
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-security-speech-01"
}
```
...
```json
{
  "image": {"type": "artifact", "artifact_id": "art_security_01"},
  "driving_audio": {"type": "artifact", "artifact_id": "art_security_speech_01"},
  "prompt": "A clear all-hands security-awareness read from this still. Keep face, points graphic, and lighting stable.",
  "duration": 8,
  "client_request_id": "opaque-security-video-01"
}
```
...
points match the supplied talking points. Report only what the host
