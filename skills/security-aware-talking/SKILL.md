---
name: "security-aware-talking"
description: "Turn user-supplied security-awareness talking points and authorized stills into one all-hands talking clip per still. This security awareness talking video studio writes a speakable security awareness talking clip for each photo, then animates a 2 to 15s all-hands talking clip. Use it for security awareness talking pack and all-hands talking video pack work that stays one photo, one clip."
---

# Security Awareness Talking Clips

Turn user-supplied security-awareness talking points and authorized
stills into one talking clip per still. Deliver 2 to 8 clips. Do not
stitch them. Speak only points already written for that all-hands
brief. Refuse phishing-drill poster packs.

## Scope and adjacent routes

Use this Skill when a security or HR team wants short talking clips
that read user-supplied all-hands awareness points from stills they
can authorize.

Route a generic presenter that is not an awareness-points read to
`talking-avatar-video`. Route a wealth product factsheet to
`wealth-product-talking`. Route a club activity script to
`club-notice-talking`. Route a housing subsidy conditions read to
`housing-subsidy-talking`. Route phishing-drill or social-engineering
poster packs out — this Skill does not make those.

## Collect the awareness points

Hard inputs are:

- at least one accessible still the host Agent can inspect — a
  presenter portrait or a points graphic that will be the first
  frame;
- security-awareness talking points the team supplied (password
  hygiene, phishing recognition, device locking, or other points
  already written for the brief);
...
Reuse already-known language, team names, and a frozen `voice_id`.
Ask only for a missing hard input. A count outside 2 to 8 is still
doable: confirm that pack size and its live cost.

Do not invent company policy, run a live phishing drill, approve a
security exception, or produce phishing-drill posters. File access is
not consent.
...
Write a labeled security-awareness talking list before any paid clone,
speech, or video. Default three slots unless the team names another
count in 2 to 8: password hygiene, phishing recognition, and device
locking — only when those points already appear in the supplied brief.
Each slot records the still, the spoken line from the supplied talking
points, intended length as a 2–15s clip, and whether it uses a catalog
...
- `model: "auto"` unless the team chose a live SKU;
...
If the team wants a cloned voice, inspect an authorized sample,
...
1. Work — one spoken awareness line per named slot
...
1. Work — one talking all-hands clip per named still
...
is a new card and a new ID. Cancel only when the team asks.
...
```text
printf '%s' '{"image":{"type":"artifact","artifact_id":"art-security-01"},"driving_audio":{"type":"artifact","artifact_id":"art-speech-01"},"prompt":"A clear all-hands security-awareness read with steady eye line and a stable camera.","duration":8,"client_request_id":"opaque-security-video-01"}' | python3 scripts/mcp_client.py call beatra.videos.animate
```
...
- For slot lists, payloads, and recovery, read
  [Security awareness talking workflow](references/workflow.md).
