...
If approved speech already exists, skip `beatra.speech.synthesize`. On
the script route, poll each `beatra.speech.synthesize` task with
`beatra.tasks.get` until terminal. If planned speech is longer than the
longest containable clip, keep a ledger of sequential segments. Each
segment has its own speech request ID, approved audio artifact, and
video request ID. Read the returned artifact plus actual audio MIME
type, duration, and size. Approve the real audio before that segment's
video admission. Refresh `image_to_video` cards and re-admit the actual
...
```json
{
  ...
  "prompt": "A restrained founder delivery with steady eye line, subtle expression, and a stable camera.",
  "duration": 8,
  "client_request_id": "opaque-founder-week-01-seg-01"
}
```
...
Later weeks reuse the frozen `voice_id` and the same portrait artifact
when it still admits.
