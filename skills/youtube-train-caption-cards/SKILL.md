...
Turn YouTube training captions into one handout still per
picked caption point. Deliver 4 to 8 stills.

## Scope and adjacent routes

Use this Skill when a training or enablement team wants handout
stills that print points from public YouTube training captions.

Route a spoken remake script to `youtube-caption-studio`. Route a
YouTube cover to `youtube-thumbnail-maker`. Route a video remake to
`viral-video-teardown-remake` when that package is installed.

This package reads YouTube training captions and makes handout
...
- the named training session being turned into a handout;
- either a public YouTube video link or search wording the host
  can open, or the original training captions already copied;
- how many cards the pack should contain, or permission to use the
  default of 4.

Reuse already-known session names, brand palette, and destination.
...
Write a labeled caption-to-handout list before any paid lookup
...
printed handout line taken from those captions, layout (caption
point, handout line, optional footer), canvas, and whether a brand
...
user wants this connection to read public YouTube training
captions, follow
[training caption lookup](references/caption-lookup.md): search,
inspect, then show a six-field lookup card and wait:

1. Work — one public YouTube video search or caption read for the
   named training session (`beatra.social.execute` with the
...
1. Work — one handout still per named slot
...
caption-backed handout line. Report only the text the host can actually
see. Treat generated small type as a review item, not as a certified
training certificate. After each terminal paid task, deliver actual bytes
...
byte-identical arguments. A changed caption, handout line,
...
- For slot lists, lookup, payloads, and recovery, read
  [training handout workflow](references/workflow.md) and
  [training caption lookup](references/caption-lookup.md).
