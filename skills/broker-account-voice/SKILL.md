---
name: "broker-account-voice"
description: "Turn written account-opening steps into one account opening voice clip per labeled cue. This account opening guidance studio records each teller guidance clip and KYC step voice from the steps the desk already wrote, then delivers 8 to 20 account opening audio files. Use it for brokerage onboarding audio that keeps one step on each clip."
---

# Account Opening Voice Pack

Turn written account-opening steps into one spoken clip per labeled
cue. Deliver 8 to 20 reads. Cover welcome, materials, and identity
first.

## Scope and adjacent routes

Use this Skill when a desk needs labeled account-opening guidance
clips from steps it already wrote.

Route a phone-tree menu to `ivr-voice-pack`. Route generic narration
to `voiceover-narration-studio`. Route shop booking notices to
`demo-booking-confirm`.

Read only the supplied step list. Do not invent a step, a document,
a yield figure, or an approval outcome.

## Collect the opening steps

Hard inputs are:

- the account-opening steps the desk already wrote;
- a pronunciation table whenever names, product terms, or branch
  names appear;
- how many clips the pack should contain, or permission to use the
  default of 10;
- likeness and voice rights when a cloned teller voice will appear.
...
Do not invent a materials line, a yield, or a reading. File access is
not consent.
...
Write a labeled account-opening voice list before any paid clone or
speech. Default ten slots unless the desk names another count in 8
to 20: welcome, materials, identity, bank-link, risk-grade, e-sign,
password, deposit, app, and complete. Each slot records the spoken
line from the written step list and whether it uses a catalog voice
or a clone.
...
If the desk wants a cloned teller voice, inspect an authorized
...
1. Work — one spoken opening-step cue per named slot
   (`beatra.speech.synthesize`).
...
pronunciation table and the written step list. Report only what the
...
```text
printf '%s' '{"input":"<the written line for this slot>","voice":"voice_...","format":"mp3","client_request_id":"opaque-broker-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```
