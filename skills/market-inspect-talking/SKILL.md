---
name: "market-inspect-talking"
description: "Turn user-supplied merchant inspection notices and authorized stills into one market inspection talking clip per still. This merchant notice talking video studio writes a speakable merchant notice talking clip for each photo, then animates a 2 to 15s merchant notice talking clip. Use it for market inspection talking pack and merchant notice talking video pack work that stays one photo, one clip."
---

# Market Inspection Talking Clips

Turn user-supplied merchant inspection notices and authorized stills
into one talking clip per still. Deliver 2 to 8 clips. Do not stitch
them. Speak only notices already written on that merchant sheet.

## Scope and adjacent routes

Use this Skill when a market-supervision office wants short talking
clips that read user-supplied merchant inspection notices from stills
they can authorize.

Route a generic presenter that is not a merchant-notice read to
`talking-avatar-video`. Route a store PA promo pack to
`store-pa-broadcast-pack`. Route a housing subsidy conditions read to
`housing-subsidy-talking`. Route a security-awareness brief to
`security-aware-talking`.

## Collect the merchant notices

Hard inputs are:

- at least one accessible still the host Agent can inspect — a
  presenter portrait or a notice graphic that will be the first
  frame;
- merchant inspection notices the office supplied (what to prepare,
  what to show, contact or window hours already on the sheet);
...
Do not invent violations, state enforcement conclusions, promise
inspection outcomes, or approve compliance. File access is not
consent.
...
Write a labeled market-inspection talking list before any paid clone,
speech, or video. Default three slots unless the office names another
count in 2 to 8: preparation, cooperation, and contact or window
hours. Each slot records the still, the spoken line from the supplied
merchant notices, intended length as a 2–15s clip, and whether it
...
- `model: "auto"` unless the office chose a live SKU;
...
If the office wants a cloned voice, inspect an authorized sample,
...
6. If the balance is insufficient — relay the official message and
   ...
   prose; keep the URL. Do not retry until the office says they have
...
1. Work — one spoken merchant-notice line per named slot
...
1. Work — one talking merchant-notice clip per named still
...
is a new card and a new ID. Cancel only when the office asks.
...
```text
printf '%s' '{"image":{"type":"artifact","artifact_id":"art-notice-01"},"driving_audio":{"type":"artifact","artifact_id":"art-speech-01"},"prompt":"A restrained merchant-notice read with steady eye line and a stable camera.","duration":8,"client_request_id":"opaque-notice-video-01"}' | python3 scripts/mcp_client.py call beatra.videos.animate
```
...
- For slot lists, payloads, and recovery, read
  [Merchant notice talking workflow](references/workflow.md).
