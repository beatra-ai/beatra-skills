# Office floor tour workflow

## Shot list

Write one slot per floor still before any paid call. Each slot names
the floor from the seller and the inspected still. Do not invent a
lobby, typical floor, or camera move. The labeled list is the free
...
```text
python3 scripts/mcp_client.py upload ./lobby.jpg --mime-type image/jpeg
```
...
```json
{
  "image": {"type": "artifact", "artifact_id": "<the uploaded floor still>"},
  "prompt": "<the written floor name for this still>",
  "duration": 5,
  "client_request_id": "opaque-floor-01"
}
```
...
Review that every clip keeps the floor shown in that still. After a
