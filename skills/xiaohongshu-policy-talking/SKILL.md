---
name: "xiaohongshu-policy-talking"
description: "Turn Xiaohongshu policy-question notes into one talking policy clip per still. This Xiaohongshu policy talking studio reads public policy-question notes, then records each already-written public-policy line as a 2 to 15s talking clip. Those policy talking clips stay separate files they can send on their own."
---

# Xiaohongshu Policy Talking

Turn Xiaohongshu policy-question notes into one talking policy clip
per still. Deliver 2 to 8 clips. Do not stitch them. Speak only
public-policy lines the desk already wrote. Do not invent a subsidy
amount, eligibility, or a personal grant.

## Scope and adjacent routes

Use this Skill when a street-office or civil-affairs desk wants
short talking clips that answer public Xiaohongshu policy-question
notes with an already-written public-policy draft.

Route Xiaohongshu hall materials stills to
`xiaohongshu-hall-faq-stills`. Route a wealth-product FAQ note read
to `xiaohongshu-wealth-faq-talking`. Route a generic presenter to
`talking-avatar-video`.

This package reads Xiaohongshu policy-question notes and makes talking
clips. It does not post on Xiaohongshu. If this connection has no
Xiaohongshu note or comment operation, work from pasted questions only.
...
- either a public Xiaohongshu note link or search wording the host
  can open, or the original policy-question notes already copied;
- already-written public-policy facts the desk supplied (what the
  clip may say; no invented subsidy, eligibility, or personal grant);
- at least one accessible still the host Agent can inspect — a
  presenter portrait or a policy graphic that will be the first
  frame;
...
Reuse already-known language, window names, and a frozen
...
Do not invent a note question, subsidy amount, eligibility rule, or
personalized grant line. File access is not consent.
...
clone, speech, or video. Default three slots unless the desk
names another count in 2 to 8. Each slot records the original
policy-question wording, the speakable line from the supplied public
draft, the still, intended length as a 2–15s clip, and whether it
uses a catalog voice, an approved track, or a clone.

That list is the free visible result from whatever the desk
already pasted. Planning is not approval.

Safe defaults:

- one `beatra.videos.animate` call per still;
- `model: "auto"` unless the desk chose a live SKU;
...
desk wants this connection to read public Xiaohongshu policy-question
notes or comments, follow
[policy note lookup](references/note-lookup.md): search,
inspect, then show a six-field lookup card and wait:

1. Work — one public Xiaohongshu note search, note read, or comment
   page for the named policy FAQ (`beatra.social.execute` with the
...
6. If the balance is insufficient — relay the official message and
   ...
   not retry until the desk says they have topped up.

Do not show `schema_hash` to the desk. A confirmed lookup does
...
If the desk wants a cloned voice, inspect an authorized sample,
...
1. Work — one spoken public-policy line per named slot
...
1. Work — one talking policy clip per named still
...
Review that each clip stays on the supplied public-policy line and
...
is a new card and a new ID. Cancel only when the desk asks.
...
```text
printf '%s' '{"input":"Open on the approved public-policy line. Keep the desk-supplied wording.","voice":"voice-ready-01","format":"mp3","speed":1.0,"model":"auto","client_request_id":"opaque-policy-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```

```text
printf '%s' '{"image":{"type":"artifact","artifact_id":"art-policy-01"},"driving_audio":{"type":"artifact","artifact_id":"art-speech-01"},"prompt":"A restrained public-policy read with steady eye line and a stable camera.","duration":8,"client_request_id":"opaque-policy-video-01"}' | python3 scripts/mcp_client.py call beatra.videos.animate
```
...
- For slot lists, lookup, payloads, and recovery, read
  [policy talking workflow](references/workflow.md) and
  [policy note lookup](references/note-lookup.md).
