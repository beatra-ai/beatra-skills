---
name: "geo-place-talking"
description: "Turn a user-supplied geography place table and authorized stills into one geography place talking clip per still. This geography place talking video studio writes a speakable place-fact talking clip for each photo, then animates a 2 to 15s place highlight talking clip. Use it for geography place talking packs that stay one photo, one clip."
---

# Geography Place Talking Clips

Turn a user-supplied geography place table and authorized stills
into one talking clip per still. Deliver 2 to 8 clips. Do not stitch
them. Speak only place facts already printed in that table.

## Scope and adjacent routes

Use this Skill when a teacher wants short talking clips that explain
geography places from a place table they already have.

Route a generic presenter that is not a place-table read to
`talking-avatar-video`. Route a club activity script read to
`club-notice-talking`. Route a unit map still pack with no talking
video to `unit-map-set`. Route a walking-route audio guide with no
video to `city-walk-audio-guide`. Route official place-script voice
with no video to `museum-audio-guide`. Route silent map motion to
`product-video-studio`.

## Collect the place table

Hard inputs are:

- at least one accessible still the host Agent can inspect — a
  presenter portrait, a map graphic, or a place photo that will be the
  first frame;
- the geography place table the teacher supplied (place names,
  regions, landforms, climate or feature facts, and any printed
  classroom notes already on that table);
- a pronunciation table whenever place names, foreign words, or coined
  labels appear;
...
Reuse already-known language, place names, and a frozen `voice_id`.
Ask only for a missing hard input. A count outside 2 to 8 is still
doable: confirm that pack size and its live cost.

Do not invent a climate claim, historical fact, exam answer, or place
detail that is not on the place table. File access is not consent.

If place names exist and the pronunciation table is empty, stop and
...
Write a labeled geography place talking list before any paid clone,
speech, or video. Default three slots unless the teacher names another
count in 2 to 8: place name intro, printed landform or feature facts,
and classroom use point already on the table. Each slot records the
still, the spoken line from the supplied place table, intended length
...
6. If the balance is insufficient — relay the official message and
   ...
   retry until the teacher says they have topped up.
...
1. Work — one spoken place line per named slot
...
1. Work — one talking geography place clip per named still
...
Review that each clip stays on the supplied place line and
...
```text
printf '%s' '{"input":"This place name is the one already printed on this geography place table.","voice":"voice-ready-01","format":"mp3","speed":1.0,"model":"auto","client_request_id":"opaque-place-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```

```text
printf '%s' '{"image":{"type":"artifact","artifact_id":"art-place-01"},"driving_audio":{"type":"artifact","artifact_id":"art-speech-01"},"prompt":"A restrained geography place read with steady eye line and a stable camera.","duration":8,"client_request_id":"opaque-place-video-01"}' | python3 scripts/mcp_client.py call beatra.videos.animate
```
...
every 24 hours per version. When a newer version is available, it
