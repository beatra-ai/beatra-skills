# Geography place talking-clip workflow

## Slot list

Write one slot per still before any paid call. Default three slots
unless the teacher names another count in 2 to 8: place name intro,
printed landform or feature facts, and classroom use point already on
the table. Each slot names the still, the spoken line from the
supplied place table, and a planned 2–15s window. Do not invent a
climate claim, historical fact, or place detail not on the table. The
...
```json
{
  "input": "This place name is the one already printed on this geography place table.",
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-place-speech-01"
}
```
...
```json
{
  "image": {"type": "artifact", "artifact_id": "art_place_01"},
  "driving_audio": {"type": "artifact", "artifact_id": "art_place_speech_01"},
  "prompt": "A restrained geography place read from this still. Keep face, map graphic, and lighting stable.",
  "duration": 8,
  "client_request_id": "opaque-place-video-01"
}
```
...
lines match the supplied place table. Report only what the host can
actually see and hear. After a returned `task_id`, poll that
