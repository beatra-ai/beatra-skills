...
Record only user-confirmed lines. Separate unit identity, page or step
title, printed unit points, language, destination, and optional visual
references. A missing line stays a named gap. Do not fill it from a
remembered textbook, a public post, or a guessed branch. Do not add a
grade, rubric, or promised score.
...
If the user supplies a teacher sample still or a unit scan, upload it
with its exact MIME type. The helper completes the upload grant and
HTTP PUT, then prints the artifact reference:

```text
python3 scripts/mcp_client.py upload ./confirmed-unit-sample.png --mime-type image/png
```
...
lines. Do not ask the model to invent a missing branch or to grade
work.
...
```text
printf '%s' '{
  "prompt":"Create the approved classroom unit mind-map still. Print only the confirmed unit title and unit points. Do not add a grade or a missing branch.",
  "model":"auto",
  "count":1,
  "canvas":{"type":"preset","tier":"2K","aspect":"3:4"},
  "client_request_id":"opaque-unit-map-01"
...
```text
printf '%s' '{"images":[{"type":"artifact","artifact_id":"accepted-page"}],"prompt":"Correct only the approved local change. Keep every other confirmed line.","count":1,"client_request_id":"opaque-unit-map-edit-01"}' | python3 scripts/mcp_client.py call beatra.images.edit
```
...
type as unread. Do not treat the still as a graded sample, a scored
student work, or a promised exam result.
