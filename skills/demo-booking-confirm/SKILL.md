---
name: "demo-booking-confirm"
description: "Turn confirmed booking facts into one booking confirmation voice clip per labeled notice. This appointment confirm audio studio writes speakable reschedule voice clips and cancel booking voice lines for a shop, then records 8 to 20 reservation confirm voice reads. Use it for shop booking voice packs that stay one line, one clip."
---

# Booking Confirmation Voice Pack

Turn confirmed shop booking facts into one spoken clip per labeled
slot. Deliver 8 to 20 reads. Cover confirmed, rescheduled, and
cancelled first.

## Scope and adjacent routes

Use this Skill when a shop needs labeled booking-notice voice clips
the staff can play or send.

Route a phone-tree menu to `ivr-voice-pack`. Route dated in-store
promo PA to `store-pa-broadcast-pack`. Route generic narration to
`voiceover-narration-studio`.

## Collect the booking facts

Hard inputs are:

- the shop or service name the guest should hear;
- confirmed booking facts the seller already wrote (what is booked,
  success / reschedule / cancel wording, times already decided);
- a pronunciation table whenever names, services, or places appear;
...
Do not invent a time, price, offer, or reading. File access is not
consent.
...
Write a labeled booking-voice list before any paid clone or speech.
Default ten slots unless the seller names another count in 8 to 20:
confirmed, rescheduled, cancelled, reminder, running late, no-show,
waitlist, arrival, deposit, and closed. Each slot records the spoken
line from confirmed facts and whether it uses a catalog voice or a
clone.
...
sentence boundaries. Write short spoken sentences.
...
1. Work — one spoken booking line per named slot
   (`beatra.speech.synthesize`).
...
pronunciation table. Report only what the host can actually hear.
...
```text
printf '%s' '{"input":"Your booking is confirmed for Saturday at three.","voice":"voice_...","format":"mp3","client_request_id":"opaque-booking-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```
