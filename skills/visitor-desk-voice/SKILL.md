---
name: "visitor-desk-voice"
description: "Turn a written visitor reception script into one visitor reception voice clip per labeled cue. This front desk voice studio records each reception script audio and visitor greeting voice from the script the desk already wrote, then delivers 8 to 20 visitor desk voice pack files. Use it for visitor reception voice packs that keep one cue on each clip."
---

# Visitor Desk Voice Pack

Turn a written visitor reception script into one spoken clip per
labeled cue. Deliver 8 to 20 reads. Cover greeting, check-in, and
escort first.

## Scope and adjacent routes

Use this Skill when a front desk needs labeled visitor-reception
clips from the reception script they already wrote.

Route generic narration to `voiceover-narration-studio`. Route a
shift handoff checklist to `shift-handoff-voice`. Route civil-affairs
material lists to `civil-affairs-voice`. Route elder checkup notices
to `elder-checkup-voice`.

Read only the supplied visitor reception script. Do not invent a
reception line, a visitor fact, or a policy not in the script.

## Collect the reception script

Hard inputs are:

- the visitor reception script the desk already wrote;
...
Do not invent a reception line, a visitor fact, or a policy not in the
script. File access is not consent.
...
Write a labeled visitor-reception voice list before any paid clone or
speech. Default ten slots unless the desk names another count in
8 to 20: greeting, check-in, badge, escort, waiting, contact,
directions, safety, policy, and farewell. Each slot records the
spoken line from the written reception script and whether it uses a
...
1. Work — one spoken visitor-reception cue per named slot
   (`beatra.speech.synthesize`).
...
pronunciation table and the written reception script. Report only
what the host can actually hear. Do not invent a reception line, a
visitor fact, or a policy not in the script.
...
```text
printf '%s' '{"input":"<the written line for this slot>","voice":"voice_...","format":"mp3","client_request_id":"opaque-visitor-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```
