# Visitor desk voice workflow

## Slot list

Write one slot per written reception-script item before any paid call.
Default ten slots unless the desk names another count in 8 to 20:
greeting, check-in, badge, escort, waiting, contact, directions,
safety, policy, and farewell. Each slot names the spoken line
from the written visitor reception script. Do not invent a reception
line, a visitor fact, or a policy not in the script. The labeled
...
```json
{
  ...
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-visitor-speech-01"
}
```
...
pronunciation table and the written reception script. Do not invent a
reception line, a visitor fact, or a policy not in the script. After
