# Holiday homework voice workflow

## Slot list

Write one slot per written homework assignment before any paid call.
Default ten slots unless the teacher names another count in 8 to 20:
chinese, math, english, reading, writing, worksheet, recitation,
project, review, and submit. Each slot names the spoken line from the
written assignment list. Do not invent a homework item, an answer, a
score, or a grade. The labeled list is the free artifact. It is not
approval.
...
```json
{
  ...
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-homework-speech-01"
}
```
...
pronunciation table and the written assignment list. Do not score the
homework. After a returned `task_id`, poll that task. If the create
