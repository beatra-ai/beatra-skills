---
name: "holiday-homework-voice"
description: "Turn a written holiday homework list into one holiday homework voice clip per labeled cue. This homework assignment voice studio records each homework instruction audio and assignment list voice from the list the teacher already wrote, then delivers 8 to 20 homework voice pack files. Use it for holiday homework clip packs that keep one assignment on each clip."
---

# Holiday Homework Voice Pack

Turn a written holiday homework list into one spoken clip per labeled
cue. Deliver 8 to 20 reads. Cover chinese, math, and english first.

## Scope and adjacent routes

Use this Skill when a teacher needs labeled holiday-homework
instruction clips from the assignment list they already wrote.

Route generic narration to `voiceover-narration-studio`. Route
full-course lesson narration to `course-narration-studio`.

Read only the supplied assignment list. Do not invent a homework
item, an answer, a score, or a grade.

## Collect the assignment list

Hard inputs are:

- the holiday homework assignment list the teacher already wrote;
- a pronunciation table whenever names, book titles, or coined
  words appear;
...
Do not invent a homework item, an answer, a score, or a grade. File
...
Write a labeled holiday-homework voice list before any paid clone or
speech. Default ten slots unless the teacher names another count in
8 to 20: chinese, math, english, reading, writing, worksheet,
recitation, project, review, and submit. Each slot records the spoken
line from the written assignment list and whether it uses a catalog
voice or a clone.
...
1. Work — one spoken homework-instruction cue per named slot
   (`beatra.speech.synthesize`).
...
pronunciation table and the written assignment list. Report only
what the host can actually hear. Do not score the homework.
...
```text
printf '%s' '{"input":"<the written line for this slot>","voice":"voice_...","format":"mp3","client_request_id":"opaque-homework-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```
