---
name: "douyin-comment-to-demo-clip"
description: "Turn Douyin comment objections into one comment talking clip per still. This douyin comment demo studio writes a speakable demo talking clip from each seller-picked objection, then animates a 2 to 15s objection demo clip. Use it for comment demo video and a douyin objection video that stay one photo, one clip."
---

# Douyin Comment Demo Clips

Turn Douyin comment objections into one talking demo clip per still.
Deliver 2 to 8 clips. Do not stitch them.

## Scope and adjacent routes

Use this Skill when a seller wants short talking clips that answer
public Douyin comment objections from stills they can authorize.

Route grouped audience language to `comment-voc-miner`. Route pin
screenshot stills to `comment-pin-card`. Route a generic talking
teaser that is not answering comments to `talking-avatar-video` or
`creator-drop-talking`.

This package reads Douyin comments and makes talking clips. It does
not post a reply on Douyin. If this connection has no Douyin comment
operation, work from pasted lines only.

## Collect the objections

Hard inputs are:

- either a public Douyin post link the host can open, or the original
  comment lines already copied;
- confirmed product or demo facts the seller already wrote (what the
  reply may say, what the still actually shows);
...
Do not invent a comment, username, like count, price, or offer. File
...
Write a labeled quote-to-demo list before any paid lookup, clone,
speech, or video. Default three slots unless the seller names another
count in 2 to 8. Each slot records the original comment line, the
speakable demo reply from confirmed facts, the still, intended length
...
3. Count — one prepaid lookup. A share URL first needs a resolve
   lookup; the comment page is a second charge. The next comments
...
1. Work — one spoken demo reply per named slot
...
1. Work — one talking demo clip per named still
...
is the first frame. Report only what the host can actually see and
hear. Do not promise perfect lip sync. Never invent a stitch, concat,
or editor tool. After each terminal paid task, deliver actual bytes
plus MIME, duration, and size when present, and
...
```text
printf '%s' '{"image":{"type":"artifact","artifact_id":"art-demo-01"},"driving_audio":{"type":"artifact","artifact_id":"art-speech-01"},"prompt":"A restrained product demo with steady eye line and a stable camera.","duration":8,"client_request_id":"opaque-demo-video-01"}' | python3 scripts/mcp_client.py call beatra.videos.animate
```
...
- For slot lists, lookup, payloads, and recovery, read
  [Douyin comment-to-demo workflow](references/workflow.md) and
