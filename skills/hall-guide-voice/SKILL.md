---
name: "hall-guide-voice"
description: "Turn a written hall window list into one hall guide voice clip per labeled cue. This window list voice studio records each hall window audio and window cue voice from the list the hall already wrote, then delivers 8 to 20 hall guide voice pack files. Use it for hall guide clip packs that keep one window on each clip."
---

# Hall Guide Voice Pack

Turn a written hall window list into one spoken clip per labeled
cue. Deliver 8 to 20 reads. Cover info desk, take-a-number, and
documents first.

## Scope and adjacent routes

Use this Skill when a service hall needs labeled window-guide clips
from the window list they already wrote.

Route generic narration to `voiceover-narration-studio`. Route a
civil-affairs materials list to `civil-affairs-voice`. Route a census
schedule to `census-notice-voice`. Route visitor-desk lines to
`visitor-desk-voice`.

Read only the supplied hall window list. Do not invent a window, a
document, an approval, or an eligibility result.

## Collect the window list

Hard inputs are:

- the hall window list the hall already wrote;
...
Do not invent a window, a document, an approval, or an eligibility
...
Write a labeled hall-guide voice list before any paid clone or
speech. Default ten slots unless the hall names another count in
8 to 20: info desk, take-a-number, documents, payment, pickup,
social-insurance, household, waiting, hours, and exit. Each slot
records the spoken line from the written window list and whether it
...
If the hall wants a cloned staff voice, inspect an authorized
...
6. If the balance is insufficient — relay the official message and
   ...
   prose; keep the URL. Do not retry until the hall says they have
...
1. Work — one spoken window-list cue per named slot
   (`beatra.speech.synthesize`).
...
pronunciation table and the written hall window list. Report only
what the host can actually hear. Do not invent a window, a document,
an approval, or an eligibility result.
...
card and a new ID. Cancel only when the hall asks.
...
```text
printf '%s' '{"input":"<the written line for this slot>","voice":"voice_...","format":"mp3","client_request_id":"opaque-hall-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```
