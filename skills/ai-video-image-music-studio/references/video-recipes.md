...
Use current models, constraints, and prices from `beatra.models.list`. Admit
the complete payload, write the shortest admitted duration (audio-led and
extend rules unchanged), and show the video admission card before creating
...
cannot start this video, and what happens if the balance is short. “Make the
clip” is not approval. Example `duration` and `resolution` values below are
placeholders; replace them with the shortest admitted duration and lowest
admitted resolution unless the user named a higher tier. After confirmation,
create one opaque `client_request_id`, call the selected billable tool exactly
