...
3. Call `beatra.models.list` with `{"capability":"video_edit"}` to confirm a current card admits the source clip's actual MIME type, dimensions, duration, byte size, and any reference media. Confirm the live duration behavior and price basis. Keep the model at `auto` unless the user chose a concrete eligible model.
4. Show the exact restyle route, source video, reference media when used, edit instruction, model behavior, and paid boundary. Freeze them with one opaque stable `client_request_id`; then submit one `beatra.videos.edit` call exactly once. Invoke only the bundled `scripts/mcp_client.py`: the MCP tool name is the CLI argument and its arguments are JSON on standard input. For example:
...
Restyling is one paid stage. A changed source video, reference media, edit instruction, model, aspect ratio, or video control is new logical paid work with a new ID and fresh approval.
