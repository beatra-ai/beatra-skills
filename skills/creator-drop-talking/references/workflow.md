# Drop talking-clip workflow

## Slot list

Write one slot per still before any paid call. Default three slots
unless the seller names another count in 2 to 8: teaser, feature, and
close. Each slot names the still, the spoken line from confirmed drop
facts, and a planned 2–15s window. Do not invent a drop date, price,
claim, or unseen product fact. The labeled list is the free artifact.
It is not approval.
...
When the seller already supplied an approved speech track for a slot,
...
```json
{
  "input": "The drop is live. Colorway one is the still. Ships this week.",
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-drop-speech-01"
}
```
...
```json
{
  "image": {"type": "artifact", "artifact_id": "art_drop_01"},
  "driving_audio": {"type": "artifact", "artifact_id": "art_drop_speech_01"},
  "prompt": "A restrained drop announcement from this still. Keep face, product, and lighting stable.",
  "duration": 8,
  "client_request_id": "opaque-drop-video-01"
}
```
