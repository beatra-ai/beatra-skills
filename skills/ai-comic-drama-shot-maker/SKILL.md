...
Default to one shot, `model: "auto"`, and a source-derived aspect ratio. Freeze a shot card before any paid call: character identity, costume, art style, scene, action, emotion, camera, composition, and the visible end state. Route by input semantics:
...
Each shot is one paid video stage. An image stage that creates an original first frame is a separate paid stage with its own request ID. A changed character identity, art style, panel, reference, prompt, model, aspect ratio, or video control is new logical paid work with a new ID and fresh approval.
