...
directly instead: the field shows how the category's strong notes open and what they claim; a pasted reference note fixes one of them exactly; the comments give the objections and the skin-type questions a beauty note has to answer; and the account's own recent notes keep the voice consistent.
...
5. Deliver what it returned and what it cost: the returned payload, its `task_id`, its
   ...
   and no dimensions to report — but it charged, and in this Skill it is the **only**
   thing that ever charges, so the number matters more here than anywhere else.
...
platform wording in `error.message`, and do not offer a different model — this Skill
does not carry a model-card tool and has no model to switch to. Do not show
`schema_hash` to the user.
