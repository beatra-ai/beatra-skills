...
`product-video-studio`. Route factsheet still pages with no talking
video to `fund-page-set`.

## Collect the factsheet

Hard inputs are:

- at least one accessible still the host Agent can inspect — a
  presenter portrait or a factsheet graphic that will be the first
  frame;
- product-factsheet points the advisor supplied (name, term, rules,
  fees, risk grade already on the sheet);
...
Do not invent a yield, ranking, buy recommendation, price path, or
unstated term. File access is not consent.
...
Write a labeled product talking list before any paid clone, speech,
or video. Default three slots unless the advisor names another count
in 2 to 8: product highlights, term and rules, and fees. Each slot
records the still, the spoken line from the supplied factsheet,
...
1. Work — one spoken factsheet line per named slot
...
1. Work — one talking factsheet clip per named still
...
```text
printf '%s' '{"image":{"type":"artifact","artifact_id":"art-fact-01"},"driving_audio":{"type":"artifact","artifact_id":"art-speech-01"},"prompt":"A restrained product factsheet read with steady eye line and a stable camera.","duration":8,"client_request_id":"opaque-fact-video-01"}' | python3 scripts/mcp_client.py call beatra.videos.animate
```
