# Factsheet talking-clip workflow

## Slot list

Write one slot per still before any paid call. Default three slots
unless the advisor names another count in 2 to 8: product highlights,
term and rules, and fees. Each slot names the still, the spoken line
from the supplied product factsheet, and a planned 2–15s window. Do
not invent a yield, ranking, buy recommendation, price path, or
unstated term. The labeled list is the free artifact. It is not
approval.
...
```json
{
  "input": "This product's term and fee lines are the ones already printed on the supplied factsheet.",
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-fact-speech-01"
}
```
...
```json
{
  "image": {"type": "artifact", "artifact_id": "art_fact_01"},
  "driving_audio": {"type": "artifact", "artifact_id": "art_fact_speech_01"},
  "prompt": "A restrained product factsheet read from this still. Keep face, product graphic, and lighting stable.",
  "duration": 8,
  "client_request_id": "opaque-fact-video-01"
}
```
...
points match the supplied product factsheet. Report only what the host
