---
name: "insurance-clause-talking"
description: "Turn user-supplied insurance policy clauses and authorized stills into one insurance clause talking clip per still. This insurance clause talking video studio writes a speakable clause explanation talking clip for each photo, then animates a 2 to 15s insurance clause talking clip. Use it for insurance clause talking packs that stay one photo, one clip."
---

# Insurance Clause Talking Clips

Turn user-supplied insurance policy clauses and authorized stills into one
talking clip per still. Deliver 2 to 8 clips. Do not stitch them. Speak
only clause explanation lines already printed in the supplied text.

## Scope and adjacent routes

Use this Skill when an insurance advisor or wealth educator wants short
talking clips that explain policy clauses they already have.

Route a generic presenter that is not a clause read to
`talking-avatar-video`. Route a wealth FAQ read to `wealth-faq-talking`.
Route a fund dividend announcement read to `fund-dividend-talking`. Route
insurance comparison stills with no talking video to
...
- at least one accessible still the host Agent can inspect — a
  presenter portrait or a clause graphic that will be the first frame;
- the insurance policy clauses the advisor supplied (coverage scope,
  waiting period, exclusions, benefit limits, and other printed wording
  they want explained);
...
Do not invent coverage, payout amounts, claim approval odds, waiting
periods, exclusions, or benefit limits that are not in the supplied
clauses. Do not promise payout or guarantee claim success. File access
is not consent.
...
Write a labeled clause talking list before any paid clone, speech, or
video. Default three slots unless the advisor names another count in 2
to 8: coverage scope, waiting period, and exclusions. Each slot records
the still, the spoken line from the supplied clauses, intended length
as a 2–15s clip, and whether it uses a catalog voice, an approved
track, or a clone.
...
1. Work — one talking clause clip per named still
...
Review that each clip stays on the supplied clause line and that the
still is the first frame. Report only what the host can actually see
and hear. Do not promise perfect lip sync. Never invent a stitch,
concat, or editor tool. After each terminal paid task, deliver actual
bytes plus MIME, duration, and size when present, and
...
```text
printf '%s' '{"input":"The waiting period is the one already printed in this clause.","voice":"voice-ready-01","format":"mp3","speed":1.0,"model":"auto","client_request_id":"opaque-clause-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```

```text
printf '%s' '{"image":{"type":"artifact","artifact_id":"art-clause-01"},"driving_audio":{"type":"artifact","artifact_id":"art-speech-01"},"prompt":"A restrained insurance clause explanation read with steady eye line and a stable camera.","duration":8,"client_request_id":"opaque-clause-video-01"}' | python3 scripts/mcp_client.py call beatra.videos.animate
```
...
- For slot lists, payloads, and recovery, read
  [Clause talking workflow](references/workflow.md).
