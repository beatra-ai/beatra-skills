---
name: "fund-report-talking"
description: "Turn a user-supplied fund quarterly report highlight sheet and authorized stills into one fund quarterly report talking clip per still. This fund quarterly report talking video studio writes a speakable report-fact talking clip for each photo, then animates a 2 to 15s quarterly highlight talking clip. Use it for fund quarterly report talking packs that stay one photo, one clip."
---

# Fund Quarterly Report Talking Clips

Turn a user-supplied fund quarterly report highlight sheet and authorized stills
into one talking clip per still. Deliver 2 to 8 clips. Do not stitch
them. Speak only report facts already printed in those highlights.

## Scope and adjacent routes

Use this Skill when a wealth advisor or fund marketer wants short
talking clips that explain quarterly report highlights they already
have.

Route a generic presenter that is not a quarterly report read to
`talking-avatar-video`. Route a product factsheet read to
`wealth-product-talking`. Route a dividend announcement read to
`fund-dividend-talking`. Route a user-supplied holding explanation to
`holding-review-talking`. Route fund page stills with no talking
...
## Collect the highlight sheet

Hard inputs are:

- at least one accessible still the host Agent can inspect — a
  presenter portrait or a report graphic that will be the first
  frame;
- the fund quarterly report highlight sheet the advisor supplied
  (fund name, reporting period, portfolio or sector facts, manager
  commentary, and any printed metrics already on that sheet);
...
Do not invent a return forecast, market direction call, buy or sell
recommendation, or metric that is not on the highlight sheet. File
...
Write a labeled quarterly report talking list before any paid clone,
speech, or video. Default three slots unless the advisor names another
count in 2 to 8: reporting period, printed portfolio or sector facts,
and manager commentary already on the sheet. Each slot records the
still, the spoken line from the supplied highlight sheet, intended
...
1. Work — one spoken highlight line per named slot
...
1. Work — one talking quarterly report clip per named still
...
Review that each clip stays on the supplied highlight line and
...
```text
printf '%s' '{"input":"The reporting period is the one already printed on this quarterly highlight sheet.","voice":"voice-ready-01","format":"mp3","speed":1.0,"model":"auto","client_request_id":"opaque-report-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```

```text
printf '%s' '{"image":{"type":"artifact","artifact_id":"art-report-01"},"driving_audio":{"type":"artifact","artifact_id":"art-speech-01"},"prompt":"A restrained quarterly report read with steady eye line and a stable camera.","duration":8,"client_request_id":"opaque-report-video-01"}' | python3 scripts/mcp_client.py call beatra.videos.animate
```
