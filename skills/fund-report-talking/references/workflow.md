# Quarterly report talking-clip workflow

## Slot list

Write one slot per still before any paid call. Default three slots
unless the advisor names another count in 2 to 8: reporting period,
printed portfolio or sector facts, and manager commentary already on
the sheet. Each slot names the still, the spoken line from the
supplied quarterly highlight sheet, and a planned 2–15s window. Do not
invent a return forecast, market direction call, buy or sell
recommendation, or metric not on the highlight sheet. The labeled list
...
```json
{
  "input": "The reporting period is the one already printed on this quarterly highlight sheet.",
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-report-speech-01"
}
```
...
```json
{
  "image": {"type": "artifact", "artifact_id": "art_report_01"},
  "driving_audio": {"type": "artifact", "artifact_id": "art_report_speech_01"},
  "prompt": "A restrained quarterly report read from this still. Keep face, report graphic, and lighting stable.",
  "duration": 8,
  "client_request_id": "opaque-report-video-01"
}
```
...
lines match the supplied quarterly highlight sheet. Report only what the
