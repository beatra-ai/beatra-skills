# Subsidy talking-clip workflow

## Slot list

Write one slot per still before any paid call. Default three slots
unless the office names another count in 2 to 8: eligibility,
materials, and window hours. Each slot names the still, the spoken
line from the supplied public subsidy conditions, and a planned
2–15s window. Do not invent eligibility, approve an application, or
grant a subsidy. The labeled list is the free artifact. It is not
approval.
...
```json
{
  "input": "<the written public-conditions line for this slot>",
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-subsidy-speech-01"
}
```
...
```json
{
  "image": {"type": "artifact", "artifact_id": "art_subsidy_01"},
  "driving_audio": {"type": "artifact", "artifact_id": "art_subsidy_speech_01"},
  "prompt": "A restrained public-conditions read from this still. Keep face, conditions graphic, and lighting stable.",
  "duration": 8,
  "client_request_id": "opaque-subsidy-video-01"
}
```
...
points match the supplied public subsidy conditions. Report only what
