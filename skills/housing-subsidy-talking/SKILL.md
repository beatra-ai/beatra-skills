---
name: "housing-subsidy-talking"
description: "Turn user-supplied public subsidy conditions and authorized stills into one housing subsidy talking clip per still. This public conditions talking video studio writes a speakable subsidy conditions talking clip for each photo, then animates a 2 to 15s public conditions talking clip. Use it for housing subsidy talking pack and subsidy talking video pack work that stays one photo, one clip."
---

# Housing Subsidy Talking Clips

Turn user-supplied public subsidy conditions and authorized stills
into one talking clip per still. Deliver 2 to 8 clips. Do not stitch
them. Speak only conditions already written on that public sheet.

## Scope and adjacent routes

Use this Skill when a street office wants short talking clips that
read user-supplied public subsidy conditions from stills they can
authorize.

Route a generic presenter that is not a subsidy-conditions read to
`talking-avatar-video`. Route a wealth product factsheet to
`wealth-product-talking`. Route a club activity script to
`club-notice-talking`. Route materials-list voice with no video to
`civil-affairs-voice`.

## Collect the public conditions

Hard inputs are:

- at least one accessible still the host Agent can inspect — a
  presenter portrait or a conditions graphic that will be the first
  frame;
- public subsidy conditions the office supplied (who, what to bring,
  window hours already on the sheet);
...
Do not invent eligibility, approve an application, or grant a
subsidy. File access is not consent.
...
Write a labeled housing-subsidy talking list before any paid clone,
speech, or video. Default three slots unless the office names another
count in 2 to 8: eligibility, materials, and window hours. Each slot
records the still, the spoken line from the supplied public
conditions, intended length as a 2–15s clip, and whether it uses a
...
1. Work — one spoken public-conditions line per named slot
...
1. Work — one talking public-conditions clip per named still
...
```text
printf '%s' '{"image":{"type":"artifact","artifact_id":"art-subsidy-01"},"driving_audio":{"type":"artifact","artifact_id":"art-speech-01"},"prompt":"A restrained public-conditions read with steady eye line and a stable camera.","duration":8,"client_request_id":"opaque-subsidy-video-01"}' | python3 scripts/mcp_client.py call beatra.videos.animate
```
...
- For slot lists, payloads, and recovery, read
  [Subsidy talking workflow](references/workflow.md).
