# Elder checkup voice workflow

## Slot list

Write one slot per written checkup-schedule item before any paid
call. Default ten slots unless the office names another count in 8
to 20: time, place, bring-items, fasting, registration, report
pickup, companion, contact, makeup day, and reminder. Each slot
names the spoken line from the written checkup schedule. Do not
diagnose, invent a result, or give medical advice. The labeled list
...
```json
{
  ...
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-elder-speech-01"
}
```
...
pronunciation table and the written checkup schedule. Do not
diagnose. After a returned `task_id`, poll that task. If the create
