---
name: "elder-checkup-voice"
description: "Turn a written elder-checkup schedule into one elder checkup voice clip per labeled cue. This checkup schedule voice studio records each elder checkup notice audio and community checkup read from the schedule the office already wrote, then delivers 8 to 20 elder checkup voice pack files. Use it for elder checkup clip packs that keep one schedule item on each clip."
---

# Elder Checkup Voice Pack

Turn a written elder-checkup schedule into one spoken clip per
labeled cue. Deliver 8 to 20 reads. Cover time, place, and what to
bring first.

## Scope and adjacent routes

Use this Skill when a street office needs labeled elder-checkup
notice clips from the schedule they already wrote.

Route generic narration to `voiceover-narration-studio`. Route a
civil-affairs materials list to `civil-affairs-voice`. Route a hall
window list to `hall-guide-voice`. Route a grid notice list to
`grid-notice-voice`. Route visitor-desk lines to `visitor-desk-voice`.

Read only the supplied checkup schedule. Do not diagnose, invent a
result, or give medical advice.

## Collect the checkup schedule

Hard inputs are:

- the elder-checkup schedule the office already wrote;
...
Do not diagnose, invent a result, or give medical advice. File access
is not consent.
...
Write a labeled elder-checkup voice list before any paid clone or
speech. Default ten slots unless the office names another count in
8 to 20: time, place, bring-items, fasting, registration, report
pickup, companion, contact, makeup day, and reminder. Each slot
records the spoken line from the written checkup schedule and whether
it uses a catalog voice or a clone.
...
sentence boundaries. Write short spoken sentences from the schedule.
...
If the office wants a cloned staff voice, inspect an authorized
...
1. Work — one spoken checkup-schedule cue per named slot
   (`beatra.speech.synthesize`).
...
pronunciation table and the written checkup schedule. Report only
what the host can actually hear. Do not diagnose or invent a result.
...
card and a new ID. Cancel only when the office asks.
...
```text
printf '%s' '{"input":"<the written line for this slot>","voice":"voice_...","format":"mp3","client_request_id":"opaque-elder-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```
