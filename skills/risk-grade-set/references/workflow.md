# Risk-grade pack workflow

## Build the pack list

Record only user-confirmed lines. Separate set identity, each named
grade and its definition in the user's words, language, destination,
and optional visual references. A missing definition stays a named
gap. Do not fill it from a brand photo, a similar product, or a
remembered regulator scale.

Default plan: one 3:4 2K still per named grade, four to eight stills.
If they name fewer than four grades, keep that count. Cap at eight.
If they want a standard set and have not numbered grades, use four
roles that already have definitions. Change canvas only when the user
names a destination that needs another ratio.
...
grade names and definitions. Do not ask the model to invent a grade, a
buy line, or a return figure.
...
```text
printf '%s' '{
  "prompt":"Create the approved risk-grade still. Print only the confirmed grade name and definition. Keep the shared pack look.",
  "model":"auto",
  "count":1,
  "canvas":{"type":"preset","tier":"2K","aspect":"3:4"},
  "client_request_id":"opaque-risk-grade-01"
}' | python3 scripts/mcp_client.py call beatra.images.generate
```

A changed definition line, language, canvas, model, or prompt is new
work: new card, new ID. Keep at most two generate tasks in flight.

For a local correction after acceptance:

```text
printf '%s' '{"images":[{"type":"artifact","artifact_id":"accepted-grade-still"}],"prompt":"Correct only the approved local change. Keep every other confirmed grade line and the shared pack look.","count":1,"client_request_id":"opaque-risk-grade-edit-01"}' | python3 scripts/mcp_client.py call beatra.images.edit
```
...
type as unread. Do not treat the still as a certified rating or a buy
recommendation.
