# CS macro card pack workflow

## Build the pack list

Record only user-confirmed lines. Separate pack identity, each named
macro title and its talk track in the team's words, language,
destination, and optional visual references. A missing point stays a
named gap. Do not fill it from a flyer photo, a similar script, or a
remembered policy line.

Default plan: one 3:4 2K still per named macro, four to eight
stills. If they name fewer than four macros, keep that count.
Cap at eight. If they want a standard set and have not numbered
macros, use four roles that already have titles. Change canvas
...
background. Do not add a refund rule, policy promise, escalation
step, or official notice the team did not supply.
...
macro titles and talk tracks. Do not ask the model to invent a script,
refund rule, policy promise, or an official notice.
...
```text
printf '%s' '{
  "prompt":"Create the approved CS macro card still. Print only the confirmed macro title and talk track. Keep the shared pack look.",
  "model":"auto",
  "count":1,
  "canvas":{"type":"preset","tier":"2K","aspect":"3:4"},
  "client_request_id":"opaque-cs-macro-card-01"
}' | python3 scripts/mcp_client.py call beatra.images.generate
```

A changed macro line, language, canvas, model, or prompt is new
work: new card, new ID. Keep at most two generate tasks in flight.

For a local correction after acceptance:

```text
printf '%s' '{"images":[{"type":"artifact","artifact_id":"accepted-macro-still"}],"prompt":"Correct only the approved local change. Keep every other confirmed macro line and the shared pack look.","count":1,"client_request_id":"opaque-cs-macro-card-edit-01"}' | python3 scripts/mcp_client.py call beatra.images.edit
```
...
original `client_request_id`. A changed macro line is new work.

Review printed lines against the confirmed pack list. Treat generated
small type as a review item, not as a published policy or refund rule.
