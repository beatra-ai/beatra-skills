# Customer onboard voice workflow

## Slot list

Write one slot per written onboarding step before any paid call.
Default ten slots unless the team names another count in 8 to 20:
welcome, identity, documents, account open, product pick, risk notice,
password, first login, support, wrap-up, and extra. Each slot names
the spoken line from the written step list. Omit a default role whose
line is missing. Do not invent an account approval, a KYC finding, or
a step not already on the list. The labeled list is the free artifact.
It is not approval.
...
Skip clone unless the team wants a cloned staff voice and can
...
```json
{
  ...
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-onboard-speech-01"
}
```
...
pronunciation table and the written onboarding step list. Do not invent
an account approval. After a returned `task_id`, poll that task. If the
