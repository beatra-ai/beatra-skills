---
name: "origin-story-module"
description: "Turn seller-supplied origin facts into one origin story module still per SKU. This origin story module studio lays out the seller-supplied place of origin and harvest lines as a product-page origin module and origin story graphic. Use it for origin story modules, farm-origin stills, and detail-page origin cards."
---

# Origin Story Modules

Make one origin story module still per SKU from facts the seller
already wrote. Deliver 4 to 8 stills. Keep each still as its own file.

## Scope and adjacent routes

Use this Skill when a seller wants short origin-story listing stills
from a written origin note and named SKUs.

Route a full photo-transform detail page to
`ecommerce-detail-module-studio`. Route Amazon selling-point A+ stills
to `amazon-a-plus-module-pack`. Route a full listing gallery to
...
- the origin facts the seller already wrote (origin place, harvest or
  process lines, and any farm name they supplied);
...
Do not invent an origin city, harvest date, certification, or face. A
farm photo is a visual reference, not a source for missing origin
...
Write a labeled origin-module list before any paid image. Default
four SKUs unless the seller names another count in 4 to 8. Each item
records the SKU, the printed origin lines from the written note,
...
1. Work — one origin story module still per named SKU
...
Review printed lines against the confirmed origin note. Report
only the text the host can actually see. Treat generated small type
as a review item, not as a certified origin claim.
...
byte-identical arguments. A changed SKU, origin line, canvas,
...
```text
printf '%s' '{"prompt":"Create the approved origin story module for the named SKU. Print only the confirmed origin lines.","model":"auto","count":1,"canvas":{"type":"preset","tier":"2K","aspect":"1:1"},"client_request_id":"opaque-origin-sku-01"}' | python3 scripts/mcp_client.py call beatra.images.generate
```
...
- For fact lists, payloads, confirmation, polling, recovery, and review,
  read [Origin-story workflow](references/workflow.md).
