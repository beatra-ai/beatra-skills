# Origin-story workflow

## Build the fact list

Record only seller-confirmed lines. Separate SKU identity, printed
origin text, origin place, language, destination, and optional
visual references. A missing line stays a named gap. Do not fill it
from a farm photo, a similar SKU, or a remembered origin tale.
...
If the seller supplies a farm or brand still, upload it with its exact
MIME type. The helper completes the upload grant and HTTP PUT, then
prints the artifact reference:

```text
python3 scripts/mcp_client.py upload ./confirmed-farm-front.png --mime-type image/png
```
...
lines. Do not ask the model to infer an origin city or harvest date.
...
```text
printf '%s' '{
  "prompt":"Create the approved origin story module. Print only the confirmed SKU name and origin lines. Do not add extra origin claims.",
  "model":"auto",
  "count":1,
  "canvas":{"type":"preset","tier":"2K","aspect":"1:1"},
  "client_request_id":"opaque-origin-sku-01"
...
```text
printf '%s' '{"images":[{"type":"artifact","artifact_id":"accepted-card"}],"prompt":"Correct only the approved local change. Keep every other confirmed line.","count":1,"client_request_id":"opaque-origin-edit-01"}' | python3 scripts/mcp_client.py call beatra.images.edit
```
...
type as unread. Do not treat the still as a certified origin claim.
