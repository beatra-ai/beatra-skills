---
name: "market-calendar-talking"
description: "Turn a user-supplied public trading calendar and authorized stills into one trading calendar talking clip per still. This public calendar talking video studio writes a speakable session calendar announcement and holiday calendar talking clip for each photo, then animates a 2 to 15s market calendar talking clip. Use it for settlement calendar talking video packs that stay one photo, one clip."
---

# Trading Calendar Talking Clips

Turn a user-supplied public trading calendar and authorized stills
into one talking clip per still. Deliver 2 to 8 clips. Do not stitch
them. Speak only dates and session facts already printed on that
calendar.

## Scope and adjacent routes

Use this Skill when a wealth advisor or hall educator wants short
talking clips that read an already-public trading calendar from
stills they can authorize.

Route a generic presenter that is not a calendar read to
`talking-avatar-video`. Route a homestay welcome to
`airbnb-welcome-avatar`. Route a product drop teaser to
`creator-drop-talking`. Route official earnings-script voice with no
...
- at least one accessible still the host Agent can inspect — a
  presenter portrait or a calendar graphic that will be the first
  frame;
- already-public trading-calendar entries the advisor supplied
  (session dates, holidays, settlement dates, printed hours);
...
Do not invent a holiday, settlement date, session hour, market take,
price path, or trade. File access is not consent.
...
Write a labeled calendar talking list before any paid clone, speech,
or video. Default three slots unless the advisor names another count
in 2 to 8: session open, holiday, and settlement. Each slot records
the still, the spoken line from the supplied public calendar, intended
...
1. Work — one spoken calendar line per named slot
...
1. Work — one talking calendar clip per named still
...
```text
printf '%s' '{"image":{"type":"artifact","artifact_id":"art-cal-01"},"driving_audio":{"type":"artifact","artifact_id":"art-speech-01"},"prompt":"A restrained calendar read with steady eye line and a stable camera.","duration":8,"client_request_id":"opaque-cal-video-01"}' | python3 scripts/mcp_client.py call beatra.videos.animate
```
