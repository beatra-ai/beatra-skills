...
Turn already-written job-fair role notes and authorized stills
into one talking clip per still. Deliver 2 to 8 clips. Keep each clip
as its own file.

## Scope and adjacent routes

Use this Skill when a recruiter wants short talking clips that walk a
student through one booth role from stills they can authorize.

Route a hiring video for an online job post to `hiring-avatar-studio`.
Route intern first-day handbook clips to `intern-onboarding-avatar`.
Route a generic presenter that is not a booth talk to
`talking-avatar-video`. Route a homestay welcome to
`airbnb-welcome-avatar`. Route a creator drop teaser to
`creator-drop-talking`.

## Collect the booth pack

Hard inputs are:

- at least one accessible still the host Agent can inspect — a
  recruiter or founder portrait, or a booth photo that will be the
  first frame;
- confirmed job-fair role facts the team already wrote (role name,
  team, booth location, next step at the booth);
...
Do not invent a salary, headcount, booth number, or face. File access
is not consent.
...
Write a labeled booth talking list before any paid clone, speech, or
video. Default three clips unless the team names another count in
2 to 8: booth welcome, role, and next step. Each item records the
still, the spoken line from confirmed role notes, intended length
...
1. Work — one spoken booth line per named clip
...
1. Work — one booth talking clip per named still
...
```text
printf '%s' '{"image":{"type":"artifact","artifact_id":"art-booth-01"},"driving_audio":{"type":"artifact","artifact_id":"art-speech-01"},"prompt":"A restrained booth welcome with steady eye line and a stable camera.","duration":8,"client_request_id":"opaque-booth-video-01"}' | python3 scripts/mcp_client.py call beatra.videos.animate
```
