---
name: "radio-drama-ad-break"
description: "Turn written radio-drama ad-break lines into one spoken bumper clip per labeled slot. This radio-drama bumper studio records each pre-roll read, post-roll read, and sponsor bumper from the copy the producer already wrote, then delivers 8 to 20 bumper audio files. Use it for radio drama ad-break voiceovers that keep one bumper on each clip."
---

# Radio Drama Ad Breaks

Turn written radio-drama ad-break lines into one spoken clip per
labeled bumper. Deliver 8 to 20 reads. Cover pre-roll and post-roll
first.

## Scope and adjacent routes

Use this Skill when a producer needs labeled bumper clips from lines
they already wrote.

Route a full multi-voice episode to `audio-drama-voice-studio`. Route
generic narration to `voiceover-narration-studio`. Route creating a
reusable voice from a sample to `voice-cloning-studio`.

Read only the supplied bumper copy. Do not add a sponsor, price,
episode title, or reading that is not on the list.

## Collect the bumper lines

Hard inputs are:

- the pre-roll, post-roll, and other bumper lines the producer
  already wrote;
...
Do not invent a sponsor line, a price, or a reading. File access is
not consent.
...
Write a labeled bumper list before any paid clone or speech.
Default ten slots unless the seller names another count in 8 to 20:
pre-roll sponsor, pre-roll disclaimer, mid-break sting, station ID,
age notice, post-roll thanks, next-episode tease, merch mention,
listener reminder, and closing sting. Each slot records the spoken
line from the written bumper copy and whether it uses a catalog voice
or a clone.
...
1. Work — one spoken bumper per named slot
   (`beatra.speech.synthesize`).
...
pronunciation table and the written bumper list. Report only what the
...
```text
printf '%s' '{"input":"<the written line for this slot>","voice":"voice_...","format":"mp3","client_request_id":"opaque-radio-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```
