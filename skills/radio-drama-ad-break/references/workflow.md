# Radio-drama bumper workflow

## Slot list

Write one slot per written bumper before any paid call. Default ten
slots unless the seller names another count in 8 to 20: pre-roll
sponsor, pre-roll disclaimer, mid-break sting, station ID, age
notice, post-roll thanks, next-episode tease, merch mention,
listener reminder, and closing sting. Each slot names the spoken
line from the written bumper copy. Do not invent a sponsor line, a
price, or a reading. The labeled list is the free artifact. It is
not approval.
...
```json
{
  ...
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-radio-speech-01"
}
```
...
pronunciation table and the written bumper list. After a returned
