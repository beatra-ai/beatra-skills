---
name: "rental-walkin-avatar"
description: "Turn seller-supplied floor-plan facts and an already-written walk-in script into one talking clip per still. This rental walk-in studio turns each authorized still into a 2 to 15s walk-in talking clip from the written line. Use it for rental walk-in videos, listing walk-in talks, and property walk-in talking clips."
---

# Walk-in Talking Clips

Turn already-written floor-plan facts, a walk-in script, and
authorized stills into one talking clip per still. Deliver 2 to 8
clips. Keep each clip as its own file.

## Scope and adjacent routes

Use this Skill when an agent wants short talking clips that walk a
viewer through named rooms from stills they can authorize.

Route a labeled room-video pack without a talking host to
`listing-room-video-pack`. Route an office-floor clip to
`office-floor-tour`. Route listing stills to
`real-estate-listing-set`. Route a homestay welcome to
`airbnb-welcome-avatar`. Do not look up public posts for a missing
address.

## Collect the walk-in pack

Hard inputs are:

- at least one accessible still the host Agent can inspect — an
  agent portrait, or a room photo that will be the first frame;
- floor-plan facts and the walk-in lines the seller already wrote;
...
Reuse already-known language, room names, and a frozen `voice_id`.
Ask only for a missing hard input. A count outside 2 to 8 is still
doable: confirm that pack size and its live cost.

Do not invent an address, price, area, school district, or face.
...
Write a labeled walk-in talking list before any paid clone, speech, or
video. Default three clips unless the seller names another count in
2 to 8: living room, bedroom, and next step. Each item records the
still, the spoken line from confirmed floor-plan facts, intended
...
1. Work — one spoken walk-in line per named clip
...
1. Work — one walk-in talking clip per named still
...
```text
printf '%s' '{"image":{"type":"artifact","artifact_id":"art-walkin-01"},"driving_audio":{"type":"artifact","artifact_id":"art-speech-01"},"prompt":"A restrained listing walk-in with steady eye line and a stable camera.","duration":8,"client_request_id":"opaque-walkin-video-01"}' | python3 scripts/mcp_client.py call beatra.videos.animate
```
...
- For clip lists, payloads, and recovery, read
  [Walk-in talking workflow](references/workflow.md).
