# Walk-in talking-clip workflow

## Clip list

Write one list item per still before any paid call. Default three
clips unless the seller names another count in 2 to 8: living room,
bedroom, and next step. Each item names the still, the spoken line
from confirmed floor-plan facts, and a planned 2–15s window. Do not
invent an address, price, area, school district, or unseen room.
The labeled list is the free artifact. It is not approval.
...
When the seller already supplied an approved speech track for a clip,
...
```json
{
  "input": "<the written walk-in line for this clip>",
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-walkin-speech-01"
}
```

Use `mp3` only when the live speech card supports it and the live
video card accepts `audio/mpeg`. If the seller requested `flac`,
...
```json
{
  "image": {"type": "artifact", "artifact_id": "art_walkin_01"},
  "driving_audio": {"type": "artifact", "artifact_id": "art_walkin_speech_01"},
  "prompt": "A restrained listing walk-in from this still. Keep face, room, and lighting stable.",
  "duration": 8,
  "client_request_id": "opaque-walkin-video-01"
}
```

Keep `model: "auto"` unless the seller chose an eligible video model.
