# Activity-script talking-clip workflow

## Slot list

Write one slot per still before any paid call. Default three slots
unless the teacher names another count in 2 to 8: event intro, time
and place, and how to join. Each slot names the still, the spoken line
from the supplied club-activity script, and a planned 2–15s window. Do
not invent a school stamp, principal name, unstated date, venue, or
official notice. The labeled list is the free artifact. It is not
approval.
...
Skip clone unless the teacher wants a cloned voice and can authorize a
...
When the teacher already supplied an approved speech track for a slot,
...
```json
{
  "input": "This club meeting uses the time and place already written on the supplied activity script.",
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-club-speech-01"
}
```

Use `mp3` only when the live speech card supports it and the live
video card accepts `audio/mpeg`. If the teacher requested `flac`,
...
the actual speech length. Split a longer spoken slot into sequential
...
```json
{
  "image": {"type": "artifact", "artifact_id": "art_club_01"},
  "driving_audio": {"type": "artifact", "artifact_id": "art_club_speech_01"},
  "prompt": "A restrained club-activity read from this still. Keep face, activity graphic, and lighting stable.",
  "duration": 8,
  "client_request_id": "opaque-club-video-01"
}
```

Keep `model: "auto"` unless the teacher chose an eligible video model.
...
points match the supplied club-activity script. Report only what the
host can actually see and hear. After a returned `task_id`, poll that
