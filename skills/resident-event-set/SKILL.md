---
name: "resident-event-set"
description: "Turn user-supplied resident event names and points into a four-to-eight still resident event set. This community event pack studio lays out each named activity as its own resident event still. Use it for activity still pack layouts, resident event graphic packs, and community activity still sets."
---

# Resident Event Pack

Make a four-to-eight still pack from event names and points the office
already supplied. Use this Skill when a street window needs a matching
resident-event still set — one still per named activity — not a lab
cover pack and not a session cover pack.

Use `lab-cover-set` when the work is one still per named assignment.
...
character. Do not look up public posts for missing event titles. Do
not invent attendance, a ranking, an official stamp, or a red-head
notice.

## Collect supplied events

Hard inputs are the exact pack name and the user-supplied event names
and points that will appear on the stills. Do not invent an event,
attendance count, ranking, or official stamp to finish a slot. Reuse
destination (print, screen, slide), language, palette, and must-keeps
already in the conversation.

Ask only for facts that change the pack:

- exact hall, street, or pack name;
- each named activity and the points the office already approved;
...
event titles. Uploading makes media available to Beatra and does
not inspect it. When the host cannot view a file, keep the user's
declared role for it.

Default to one still per named activity, four to eight stills in
the pack. If they name fewer than four activities, keep that count.
Cap the pack at eight. If they want a standard set and have not
numbered activities, plan four stills from the names already given.
Omit a line whose fact is missing and keep that gap on the plan so
the office can add it later.

Keep the same type, palette, and layout across the pack so the stills
read as one resident event set.

## Plan the free pack list

Write a labeled pack list before any paid image. For each activity
record the printed lines, language, layout (event name, points,
...
print hall posters. Change canvas only when the user names a
...
Read [resident event pack workflow](references/workflow.md) when writing
the payload or recovering a task.

## Confirm once, create by activity

Planning is free. Before the first billable call, show one current
production card that lists every named activity still, and wait:

1. Work — one activity still per named event, a generate
   call (`beatra.images.generate`).
2. Credits — the live `text_to_image` price just read. Do not reuse a
   remembered number.
3. Count — one paid call per activity still. This card lists every
   slot in the pack. Do not batch several activities into one
   `count`. This approval does not cover retries.
4. Identity — one new opaque `client_request_id` per still. A changed
   prompt, event line, file, model, or canvas mints a new ID.
...
item, not as an official notice or an attendance record.

Deliver the stills in activity order, the pack list, observed
...
```text
printf '%s' '{"prompt":"Create the approved resident-event still for the named activity. Print only the confirmed event name and points. Keep the shared pack look.","model":"auto","count":1,"canvas":{"type":"preset","tier":"2K","aspect":"3:4"},"client_request_id":"opaque-resident-event-01"}' | python3 scripts/mcp_client.py call beatra.images.generate
```
...
- For pack lists, payloads, confirmation, polling, recovery, and review,
  read [resident event pack workflow](references/workflow.md).
