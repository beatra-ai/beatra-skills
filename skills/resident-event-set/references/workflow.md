# Resident event pack workflow

## Build the pack list

Record only user-confirmed lines. Separate pack identity, each named
activity and its points in the office's words, language, destination,
and optional visual references. A missing point stays a named gap.
Do not fill it from a flyer photo, a similar event, or a remembered
attendance count.

Default plan: one 3:4 2K still per named activity, four to eight
stills. If they name fewer than four activities, keep that count.
Cap at eight. If they want a standard set and have not numbered
activities, use four roles that already have names. Change canvas
...
background. Do not add an official stamp or red-head the office did
not supply.
...
event names and points. Do not ask the model to invent an activity,
attendance, or an official notice.
...
```text
printf '%s' '{
  "prompt":"Create the approved resident-event still. Print only the confirmed event name and points. Keep the shared pack look.",
  "model":"auto",
  "count":1,
  "canvas":{"type":"preset","tier":"2K","aspect":"3:4"},
  "client_request_id":"opaque-resident-event-01"
}' | python3 scripts/mcp_client.py call beatra.images.generate
```

A changed event line, language, canvas, model, or prompt is new
work: new card, new ID. Keep at most two generate tasks in flight.

For a local correction after acceptance:

```text
printf '%s' '{"images":[{"type":"artifact","artifact_id":"accepted-event-still"}],"prompt":"Correct only the approved local change. Keep every other confirmed event line and the shared pack look.","count":1,"client_request_id":"opaque-resident-event-edit-01"}' | python3 scripts/mcp_client.py call beatra.images.edit
```
...
type as unread. Do not treat the still as an official notice or an
attendance record.
