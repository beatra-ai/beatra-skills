---
name: "credit-rights-voice"
description: "Turn a written credit-card benefits table into one credit-rights voice clip per labeled cue. This benefits-table voice studio records each card-rights audio and rights-table voice from the list the desk already wrote, then delivers 8 to 20 credit-rights voice pack files. Use it for card-rights clip packs that keep one table item on each clip."
---

# Credit Rights Voice Pack

Turn a written credit-card benefits table into one spoken clip per
labeled cue. Deliver 8 to 20 reads. Cover card intro, annual fee,
points, and lounge access first.

## Scope and adjacent routes

Use this Skill when a finance desk needs labeled credit-rights clips
from the benefits table they already wrote.
...
Read only the supplied benefits table. Do not invent an approval
guarantee, an eligibility finding, or a benefit not already on the
table.

## Collect the benefits table

Hard inputs are:

- the credit-card benefits table the desk already wrote;
- a pronunciation table whenever card names, partner brands, or coined
  words appear;
...
Do not invent an approval guarantee, an eligibility finding, or a
benefit not already on the table. File access is not consent.
...
Write a labeled credit-rights voice list before any paid clone or
speech. Default ten slots unless the desk names another count in
8 to 20: card intro, annual fee, points, lounge, insurance, travel,
dining, shopping, activation, wrap-up, and extra. Each slot records
the spoken line from the written benefits table and whether it uses a
catalog voice or a clone. Omit a default role whose line is missing on
the table.
...
1. Work — one spoken benefits-table cue per named slot
   (`beatra.speech.synthesize`).
...
pronunciation table and the written benefits table. Report only
what the host can actually hear. Do not invent an approval guarantee.
