# Credit rights voice workflow

## Slot list

Write one slot per written benefits-table item before any paid call.
Default ten slots unless the desk names another count in 8 to 20:
card intro, annual fee, points, lounge, insurance, travel, dining,
shopping, activation, wrap-up, and extra. Each slot names the spoken
line from the written benefits table. Omit a default role whose line
is missing. Do not invent an approval guarantee, an eligibility
finding, or a benefit not already on the table. The labeled list is
the free artifact. It is not approval.
...
```json
{
  ...
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-rights-speech-01"
}
```
...
pronunciation table and the written benefits table. Do not invent an
approval guarantee. After a returned `task_id`, poll that task. If the
