# Wealth open calendar pack workflow

## Build the pack list

Record only user-confirmed lines. Separate set identity, each named
open window and its dates in the user's words, language, destination,
and optional visual references. A missing date stays a named
gap. Do not fill it from a brand photo, a similar product, or a
remembered market calendar.

Default plan: one 16:9 2K still per named open window, four to eight
stills. If they name fewer than four windows, keep that count. Cap at
eight. If they want a standard set and have not numbered windows, use
four roles that already have dates. Change canvas only when the user
...
product names and open dates. Do not ask the model to invent a window,
a buy line, or a return figure.
...
```text
printf '%s' '{
  "prompt":"Create the approved open-window calendar still. Print only the confirmed product name and open dates. Keep the shared pack look.",
  "model":"auto",
  "count":1,
  "canvas":{"type":"preset","tier":"2K","aspect":"16:9"},
  "client_request_id":"opaque-wealth-open-cal-01"
...
```text
printf '%s' '{"images":[{"type":"artifact","artifact_id":"accepted-calendar-still"}],"prompt":"Correct only the approved local change. Keep every other confirmed window line and the shared pack look.","count":1,"client_request_id":"opaque-wealth-open-cal-edit-01"}' | python3 scripts/mcp_client.py call beatra.images.edit
```
