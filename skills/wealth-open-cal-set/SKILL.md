---
name: "wealth-open-cal-set"
description: "Turn user-supplied open windows into a four-to-eight still wealth open calendar. This open-period calendar set studio lays out each named fund open window calendar as its own still. Use it for open window calendar stills, wealth calendar stills, and an open calendar pack."
---

# Wealth Open Calendar

Make a four-to-eight still pack from open windows the user already
supplied. Use this Skill when a wealth session needs an open-period
calendar set — one still per named window — not a session cover set
and not a page-by-page factsheet.

Use `wealth-cover-set` when the work is one still per named session
cover. Use `risk-grade-set` when the work is one still per named risk
grade. Use `fund-page-set` when the work is one prospectus still per
named page. Do not look up public posts or market quotes for missing
open dates.

## Collect supplied windows

Hard inputs are the exact set name and the user-supplied open windows
that will appear on the calendars. Do not invent a window, date, buy
or sell line, return figure, ranking, or product pick to finish a
slot. Reuse destination (slide, screen, print), language, palette,
and must-keeps already in the conversation.

Ask only for facts that change the pack:

- exact set, series, or product name;
- each named window and the open dates the user already approved;
...
open dates. Uploading makes media available to Beatra and does not
inspect it. When the host cannot view a file, keep the user's
declared role for it.

Default to one still per named open window, four to eight stills in
the pack. If they name fewer than four windows, keep that count. Cap
the pack at eight. If they want a standard set and have not numbered
windows, plan four stills from the windows already given. Omit a
...
read as one wealth open calendar.

## Plan the free pack list

Write a labeled pack list before any paid image. For each window
record the printed lines, language, layout (product name, open dates,
...
as `auto` and `count` at `1` per still. Default canvas is 16:9 2K for
slide and screen calendars. Change canvas only when the user names a
...
Read [wealth open calendar pack workflow](references/workflow.md) when
...
production card that lists every named open-window calendar, and wait:

1. Work — one open-window calendar still per named window, a generate
   call (`beatra.images.generate`).
2. Credits — the live `text_to_image` price just read. Do not reuse a
   remembered number.
3. Count — one paid call per window still. This card lists every slot
   in the pack. Do not batch several windows into one `count`. This
   approval does not cover retries.
4. Identity — one new opaque `client_request_id` per still. A changed
   prompt, window line, file, model, or canvas mints a new ID.
...
item, not as a certified return or a buy recommendation.

Deliver the stills in window order, the pack list, observed dimensions
...
```text
printf '%s' '{"prompt":"Create the approved open-window calendar still for the named window. Print only the confirmed product name and open dates. Keep the shared pack look.","model":"auto","count":1,"canvas":{"type":"preset","tier":"2K","aspect":"16:9"},"client_request_id":"opaque-wealth-open-cal-01"}' | python3 scripts/mcp_client.py call beatra.images.generate
```
...
- For pack lists, payloads, confirmation, polling, recovery, and review,
  read [wealth open calendar pack workflow](references/workflow.md).
