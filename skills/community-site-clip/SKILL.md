---
name: "community-site-clip"
description: "Turn authorized community event site photos and office-supplied facts into one community site clip per photo. This event site clip studio makes a silent 2-15s site photo clip from each field still clip. Use it for community event clip sets that stay one photo one clip."
---

# Community Site One-Shot Clips

Turn authorized community event site photos and office-supplied facts
into one short clip per photo. Deliver the clips in photo order. Do
not stitch them.

## Scope and adjacent routes

Use this Skill when a street office wants one short motion clip for
each event-site photo they already captured.

Route a talking recap from one photo plus a script to
`wrong-item-talking`. Route assembly stills to
`assembly-one-step-clip`. Route a generic still that is not a labeled
event site to `image-to-motion`. Route classroom boards to
`blackboard-one-shot`. Do not look up public posts for missing event
facts.
...
- at least one accessible event-site photo the host Agent can inspect,
  in capture order;
- office-supplied facts that name what each photo shows (event,
  location, moment, or sequence number).

Do not invent attendance, a ranking, an official stamp, or a spoken
line. A photo is not a source for unstated wording. Reuse event name,
...
2. Write a labeled one-clip plan from the office's facts and what the
...
4. Show one current production card and wait before any video
   ...
   1. Work — one community-site clip per named photo, a generate call
   ...
   6. If the balance is insufficient — relay the official message and
      ...
      not retry until the office says they have topped up.
...
6. Deliver the clips in photo order with actual dimensions, duration,
   usage, and `billing.net_charged_credits`. Review site identity,
...
reuse a photo outside this event set.
...
do not poll; reconcile before minting a new ID. A changed photo,
prompt, duration, or model is new work and a new card.
...
- For clip plans, payloads, confirmation, polling, recovery, and
  review, read [Community site one-shot workflow](references/workflow.md).
