# Community site one-shot workflow

## Build the clip list

Record only office-supplied facts. Separate event identity when given,
each photo in capture order, the site moment that photo shows,
language, and destination. A missing fact stays a named gap. Do not fill
it from a similar event, a remembered attendance count, or what the
photo "probably" shows.
...
```text
python3 scripts/mcp_client.py upload ./community-site-01.jpg --mime-type image/jpeg
```
...
```text
printf '%s' '{
  "image": {"type": "artifact", "artifact_id": "art-community-site-01"},
  "prompt": "A restrained camera hold on this community event site frame. Keep the shown place and moment in place. Do not add unstated attendance or official marks.",
  "duration": 5,
  "client_request_id": "opaque-community-site-01"
...
Cancel only when the office asks, then wait for a terminal state.

Review visible site identity against the labeled plan. Report unread
