# Civil affairs voice workflow

## Slot list

Write one slot per written materials item before any paid call.
Default ten slots unless the office names another count in 8 to 20:
marriage, birth, household, social-insurance, funeral, disability,
veteran, adoption, assistance, and appointment. Each slot names the
spoken line from the written materials list. Do not invent a
document, an approval, a grant, or an eligibility result. The labeled
...
```json
{
  ...
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-civil-speech-01"
}
```
...
pronunciation table and the written materials list. Do not approve an
application. After a returned `task_id`, poll that task. If the create
