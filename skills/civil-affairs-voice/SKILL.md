---
name: "civil-affairs-voice"
description: "Turn a written civil-affairs materials list into one civil affairs voice clip per labeled cue. This materials list voice studio records each civil affairs instruction audio and window materials voice from the list the office already wrote, then delivers 8 to 20 civil affairs voice pack files. Use it for civil affairs clip packs that keep one materials item on each clip."
---

# Civil Affairs Voice Pack

Turn a written civil-affairs materials list into one spoken clip per
labeled cue. Deliver 8 to 20 reads. Cover marriage, birth, and
household first.

## Scope and adjacent routes

Use this Skill when a street office needs labeled civil-affairs
instruction clips from the materials list they already wrote.

Route generic narration to `voiceover-narration-studio`. Route a
holiday homework list to `holiday-homework-voice`. Route a homeroom
weekly list to `homeroom-week-voice`. Route a hall window list to
`hall-guide-voice`. Route visitor-desk lines to `visitor-desk-voice`.

Read only the supplied materials list. Do not invent a document, an
approval, a grant, or an eligibility result.

## Collect the materials list

Hard inputs are:

- the civil-affairs materials list the office already wrote;
...
Do not invent a document, an approval, a grant, or an eligibility
result. File access is not consent.
...
Write a labeled civil-affairs voice list before any paid clone or
speech. Default ten slots unless the office names another count in
8 to 20: marriage, birth, household, social-insurance, funeral,
disability, veteran, adoption, assistance, and appointment. Each slot
records the spoken line from the written materials list and whether it
uses a catalog voice or a clone.
...
1. Work — one spoken materials-list cue per named slot
   (`beatra.speech.synthesize`).
...
pronunciation table and the written materials list. Report only
what the host can actually hear. Do not approve an application.
...
```text
printf '%s' '{"input":"<the written line for this slot>","voice":"voice_...","format":"mp3","client_request_id":"opaque-civil-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```
