---
name: "incident-brief-voice"
description: "Turn a written incident briefing script into one incident brief voice clip per labeled cue. This incident brief voice studio records each accident notice audio and safety brief read from the script the office already wrote, then delivers 8 to 20 incident brief voice pack files. Use it for incident brief clip packs that keep one brief line on each clip."
---

# Incident Brief Voice Pack

Turn a written incident briefing script into one spoken clip per
labeled cue. Deliver 8 to 20 reads. Cover time, place, and the
written status first.

## Scope and adjacent routes

Use this Skill when a safety or operations desk needs labeled
incident-brief clips from the briefing script they already wrote.
...
elder-checkup schedule to `elder-checkup-voice`. Route a grid notice
list to `grid-notice-voice`.

Read only the supplied briefing script. Do not invent a cause,
assign blame, or add an outcome that is not already in the written
brief.

## Collect the briefing script

Hard inputs are:

- the incident briefing script the office already wrote;
...
Do not invent a cause, assign blame, or add an outcome missing from
the brief. File access is not consent.
...
Write a labeled incident-brief voice list before any paid clone or
speech. Default ten slots unless the desk names another count in
8 to 20: opening, time, place, what-happened, status, actions-taken,
audience, contact, next-steps, and reminder. Each slot records the
spoken line from the written briefing script and whether it uses a
catalog voice or a clone.
...
1. Work — one spoken brief cue per named slot
   (`beatra.speech.synthesize`).
...
pronunciation table and the written briefing script. Report only
what the host can actually hear. Do not invent a cause, assign
blame, or add an outcome missing from the brief.
...
```text
printf '%s' '{"input":"<the written line for this slot>","voice":"voice_...","format":"mp3","client_request_id":"opaque-incident-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```
