---
name: "intern-onboarding-avatar"
description: "Turn authorized stills and already-written intern onboarding notes into one intern onboarding talk per still. This intern welcome video studio writes a speakable first-day talking clip from the handbook, then animates each still into a 2 to 15s onboarding talking clip. Use it for intern briefing videos, intern first-day video, and intern onboarding video that stay one photo, one clip."
---

# Intern Onboarding Talking Clips

Turn already-written intern onboarding notes and authorized stills
into one talking clip per still. Deliver 2 to 8 clips. Keep each clip
as its own file.

## Scope and adjacent routes

Use this Skill when a recruiter or hiring manager wants short talking
clips that walk an intern through first-day facts from stills they
can authorize.

Route a hiring video for an open role to `hiring-avatar-studio`.
Route a generic presenter that is not intern onboarding to
`talking-avatar-video`. Route a homestay welcome to
`airbnb-welcome-avatar`. Route a multi-lesson lecture series to
`course-video-studio`. Route a creator drop teaser to
`creator-drop-talking`.

## Collect the onboarding pack

Hard inputs are:

- at least one accessible still the host Agent can inspect — a
  recruiter or founder portrait, or a workplace photo that will be
  the first frame;
- confirmed intern onboarding facts the team already wrote (hours,
  first-day checklist, workplace location, safety notes, next step);
...
Reuse already-known language, workplace names, and a frozen
`voice_id`. Ask only for a missing hard input. A count outside 2 to 8
is still doable: confirm that pack size and its live cost.

Do not invent a stipend, schedule, badge code, or face. File access
is not consent.
...
Write a labeled intern talking list before any paid clone, speech, or
video. Default three clips unless the team names another count in
2 to 8: welcome, first day, and next step. Each item records the
still, the spoken line from confirmed handbook facts, intended length
...
1. Work — one spoken intern line per named clip
...
1. Work — one intern talking clip per named still
...
```text
printf '%s' '{"image":{"type":"artifact","artifact_id":"art-intern-01"},"driving_audio":{"type":"artifact","artifact_id":"art-speech-01"},"prompt":"A restrained intern welcome with steady eye line and a stable camera.","duration":8,"client_request_id":"opaque-intern-video-01"}' | python3 scripts/mcp_client.py call beatra.videos.animate
```
...
- For clip lists, payloads, and recovery, read
  [Intern onboarding talking workflow](references/workflow.md).
