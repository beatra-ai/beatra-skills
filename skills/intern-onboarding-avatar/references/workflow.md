# Intern onboarding talking-clip workflow

## Clip list

Write one list item per still before any paid call. Default three
clips unless the team names another count in 2 to 8: welcome, first
day, and next step. Each item names the still, the spoken line from
confirmed handbook facts, and a planned 2–15s window. Do not invent
a stipend, schedule, badge code, or unseen workplace fact. The
...
returned `voice_id` frozen for later speech clips.

## Speech admission

When the team already supplied an approved speech track for a clip,
...
production card, then submit once per clip:

```json
{
  "input": "<the written intern line for this clip>",
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-intern-speech-01"
}
```
...
```json
{
  "image": {"type": "artifact", "artifact_id": "art_intern_01"},
  "driving_audio": {"type": "artifact", "artifact_id": "art_intern_speech_01"},
  "prompt": "A restrained intern welcome from this still. Keep face, workplace, and lighting stable.",
  "duration": 8,
  "client_request_id": "opaque-intern-video-01"
}
```
