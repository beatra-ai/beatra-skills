...
3. Call `beatra.models.list` with `{"capability":"video_extend"}` to confirm a current card admits the source clip's actual MIME type, dimensions, duration, and byte size. Confirm that card's live final-duration maximum, direction control, optional reference limits, and price basis. Keep the model at `auto` unless the user chose a concrete eligible model.
4. Choose the extension duration. The request `duration` is the final returned-video duration, not seconds to add: it must be an integer that exceeds the trusted source duration and stays within the live maximum. If the source already meets or exceeds the current maximum, this route cannot enter and the user must be told plainly. Show the exact source video, continuation direction, next beat, final duration, model behavior, and paid boundary. Freeze them with one opaque stable `client_request_id`; then submit one `beatra.videos.extend` call exactly once. Invoke only the bundled `scripts/mcp_client.py`: the MCP tool name is the CLI argument and its arguments are JSON on standard input. For example:
...
An extension is one paid stage. A changed source video, continuation direction, next beat, final duration, model, or control is new logical paid work with a new ID and fresh approval. The `duration` is the final returned-video duration each time; it never means seconds to add.
