# Hall FAQ still workflow

## Slot list

Write one slot per user-picked hall-faq complaint before any paid
call. Default four slots unless the user names another count in 4
to 8. Each slot names the original complaint wording, the printed
materials line from confirmed official facts, layout, and canvas.
Do not invent a complaint, username, or materials line. The labeled
list is the free artifact. It is not approval.

If the user wants this connection to read a public Xiaohongshu
note or search, follow [hall FAQ lookup](hall-faq-lookup.md) on its
own card first.
...
confirmed materials line. After a returned `task_id`, poll that task.
...
byte-identical arguments. A changed complaint, materials line, still,
