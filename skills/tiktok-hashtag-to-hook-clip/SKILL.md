...
Turn a TikTok hashtag trend into one talking hook clip per still.
Deliver 2 to 8 clips. Do not stitch them.

## Scope and adjacent routes

Use this Skill when an operator wants short talking clips that open
on a public TikTok hashtag from presenter or product stills they
already have.

Route a full same-day post with cover and caption to
`hot-topic-content-maker`. Route Douyin hot-search hooks to
`douyin-hot-to-hook-clip`. Route a generic talking teaser that is
not answering a hashtag to `talking-avatar-video` or
`creator-drop-talking`.

This package reads TikTok hashtag rankings and head posts and
makes talking clips. It does not post on TikTok. If this connection
has no TikTok hashtag or video-search operation, work from pasted
tags only.
...
- either permission to read the current public TikTok hashtag
  board, or the original hashtag lines already copied;
- confirmed brand or product facts the operator already wrote (what
  the hook may say, what the still actually shows);
...
Do not invent a hashtag, rank, heat score, price, or offer. File
...
Write a labeled tag-to-hook list before any paid lookup, clone,
speech, or video. Default three slots unless the operator names
another count in 2 to 8. Each slot records the original hashtag
wording, the speakable hook from confirmed facts, the still,
intended length as a 2–15s clip, and whether it uses a catalog
...
## Look up the hashtag board on its own card

A request to “see what’s trending” does not authorize a lookup.
When the operator wants this connection to read public TikTok
hashtag rankings or head posts, follow
[hashtag lookup](references/hashtag-lookup.md): search, inspect,
then show a six-field lookup card and wait:

1. Work — one public TikTok hashtag-trend page or video-search page
...
3. Count — one prepaid lookup. A hashtag-trend page is its own
   billed execute. A video search for head posts is another. The
   next page is another charge.
...
```text
printf '%s' '{"query":"hashtag","platform":"tiktok","capability_family":"trends"}' | python3 scripts/mcp_client.py call beatra.social.tools.search
```
...
- For slot lists, lookup, payloads, and recovery, read
  [hashtag hook workflow](references/workflow.md) and
  [hashtag lookup](references/hashtag-lookup.md).
