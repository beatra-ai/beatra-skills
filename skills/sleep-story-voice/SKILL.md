---
name: "sleep-story-voice"
description: "Turn already-written short sleep stories into one spoken clip per labeled story. This sleep story voice studio records each short sleep-story voice from the pages the producer already wrote. Use it for sleep stories, bedtime story reads, bedtime narration, and short sleep-story voice packs."
---

# Sleep Story Voice

Turn already-written short sleep stories into one spoken clip per
labeled story. Deliver 8 to 20 reads. Cover opening hush, first
scene, and landing first.

## Scope and adjacent routes

Use this Skill when a producer needs labeled short sleep-story
clips they can import into a player.

Route a long chapter serial to `ai-audiobook-narration`. Route a
radio ad break to `radio-drama-ad-break`. Route generic narration
to `voiceover-narration-studio`. Route sales voicemail to
`sales-voicemail-pack`.

Read only the supplied short stories. Do not add a plot, a
character name, a city, or a reading that is not on the list.

## Collect the short stories

Hard inputs are:

- the short sleep stories the producer already wrote;
...
Do not invent a plot, a character name, a city, or a reading. File
access is not consent.

If names exist and the pronunciation table is empty, stop and collect
the readings.

A long serial is a different job. Keep this pack on short stories.

## Plan the free listen list

Write a labeled sleep-story list before any paid clone or speech.
Default ten slots unless the producer names another count in 8 to
20: opening hush, first scene, turning, landing, close, next-night,
rain bed, night walk, cabin, and river. Each slot records the spoken
text from the written short story and whether it uses a catalog
voice or a clone. Leave a missing story off the paid list.
...
If the producer wants a cloned voice, inspect an authorized sample,
...
6. If the balance is insufficient — relay the official message and
   ...
   prose; keep the URL. Do not retry until the producer says they
...
1. Work — one spoken sleep story per named slot
   (`beatra.speech.synthesize`).
...
pronunciation table and the written sleep-story list. Report only
what the host can actually hear.
...
byte-identical arguments. A changed story, voice, or speed is a new
card and a new ID. Cancel only when the producer asks.
...
```text
printf '%s' '{"input":"<the written short story for this slot>","voice":"voice_...","format":"mp3","client_request_id":"opaque-sleep-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```
...
- For listen lists, payloads, and recovery, read
