# Clearance-card workflow

## Build the fact list

Record only seller-confirmed lines. Separate SKU identity, printed
clearance text, dates the seller already wrote, language, destination,
and optional visual references. A missing line stays a named gap. Do
not fill it from a product photo, a similar SKU, or a remembered
promotion.

Default plan: one square 2K still per SKU. Change canvas only when the
seller names a destination that needs another ratio. Default four SKUs
unless the seller names another count in 4 to 8.
...
lines. Do not ask the model to invent a discount.
...
```text
printf '%s' '{
  "prompt":"Create the approved clearance explanation card. Print only the confirmed SKU name and clearance lines.",
  "model":"auto",
  "count":1,
  "canvas":{"type":"preset","tier":"2K","aspect":"1:1"},
  "client_request_id":"opaque-clearance-sku-01"
...
```text
printf '%s' '{"images":[{"type":"artifact","artifact_id":"accepted-card"}],"prompt":"Correct only the approved local change. Keep every other confirmed line.","count":1,"client_request_id":"opaque-clearance-edit-01"}' | python3 scripts/mcp_client.py call beatra.images.edit
```
...
type as unread. Do not treat the still as a certified sale notice.
