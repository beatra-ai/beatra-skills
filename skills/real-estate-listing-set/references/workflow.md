# Listing-still workflow

## Build the fact list

Record only seller-confirmed lines. Separate listing identity, printed
layout text, room name, language, destination, and optional
visual references. A missing line stays a named gap. Do not fill it
from a room photo, a similar listing, or a remembered price.

Default plan: one square 2K still per room. Change canvas only when
the seller names a destination that needs another ratio. Default four
rooms unless the seller names another count in 4 to 8.

## Upload optional references

If the seller supplies a room or brand still, upload it with its
exact MIME type. The helper completes the upload grant and HTTP PUT,
then prints the artifact reference:

```text
python3 scripts/mcp_client.py upload ./confirmed-living-front.png --mime-type image/png
```
...
lines. Do not ask the model to infer an address or asking price.

## Freeze and submit

Show the six-field production card from the root before the first
billable generate. After approval, mint one opaque
`client_request_id` per room. Submit once:

```text
printf '%s' '{
  "prompt":"Create the approved listing still. Print only the confirmed room name and layout lines. Do not add extra listing claims.",
  "model":"auto",
  "count":1,
  "canvas":{"type":"preset","tier":"2K","aspect":"1:1"},
  "client_request_id":"opaque-listing-room-01"
...
```text
printf '%s' '{"images":[{"type":"artifact","artifact_id":"accepted-card"}],"prompt":"Correct only the approved local change. Keep every other confirmed line.","count":1,"client_request_id":"opaque-listing-edit-01"}' | python3 scripts/mcp_client.py call beatra.images.edit
```
...
type as unread. Do not treat the still as a certified listing claim.
