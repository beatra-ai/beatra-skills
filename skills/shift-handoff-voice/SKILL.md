---
name: "shift-handoff-voice"
description: "Turn a written shift handoff checklist into one shift handoff voice clip per labeled cue. This shift handover voice studio records each handover checklist audio and shift change voice from the list the desk already wrote, then delivers 8 to 20 shift handoff clip files. Use it for shift handoff voice packs that keep one cue on each clip."
---

# Shift Handoff Voice Pack

Turn a written shift handoff checklist into one spoken clip per
labeled cue. Deliver 8 to 20 reads. Cover outgoing, incoming, and
unfinished work first.

## Scope and adjacent routes

Use this Skill when a desk needs labeled shift-handoff clips from the
checklist they already wrote.

Route generic narration to `voiceover-narration-studio`. Route a
homeroom week list to `homeroom-week-voice`. Route a first-week
manager list to `new-manager-voice`. Route an incident brief to
`incident-brief-voice`.

Read only the supplied shift-handoff checklist. Do not invent a
missing handoff item.

## Collect the shift-handoff list

Hard inputs are:

- the shift-handoff checklist the desk already wrote;
...
Do not invent a missing handoff item. File access is not consent.
...
Write a labeled shift-handoff voice list before any paid clone or
speech. Default ten slots unless the desk names another count in
8 to 20: outgoing, incoming, unfinished, tools, sites, contacts,
hazards, inventory, reminder, and follow-up. Each slot records the
spoken line from the written checklist and whether it uses a catalog
voice or a clone.
...
1. Work — one spoken shift-handoff cue per named slot
   (`beatra.speech.synthesize`).
...
pronunciation table and the written shift-handoff checklist. Report
only what the host can actually hear. Do not invent a missing
handoff item.
...
```text
printf '%s' '{"input":"<the written line for this slot>","voice":"voice_...","format":"mp3","client_request_id":"opaque-handoff-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```
