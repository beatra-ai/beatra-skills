# Shift handoff voice workflow

## Slot list

Write one slot per written shift-handoff item before any paid call.
Default ten slots unless the desk names another count in 8 to 20:
outgoing, incoming, unfinished, tools, sites, contacts, hazards,
inventory, reminder, and follow-up. Each slot names the spoken line
from the written shift-handoff checklist. Do not invent a missing
handoff item. The labeled list is the free artifact. It is not
approval.
...
```json
{
  ...
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-handoff-speech-01"
}
```
...
pronunciation table and the written shift-handoff checklist. Do not
invent a missing handoff item. After a returned `task_id`, poll that
...
with byte-identical arguments. A changed line, voice, or speed is a
new card and a new ID.
