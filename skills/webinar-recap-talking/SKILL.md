...
Turn an authorized presenter still and a post-meeting external script
into one talking recap clip per still. Deliver 2 to 8 clips. Do not
stitch them.

## Scope and adjacent routes

Use this Skill when an enterprise training or customer-enablement team
needs short talking clips that recap a customer meeting or webinar from
a script they already wrote for external use.

Route a product drop announcement to `creator-drop-talking`. Route
official earnings reads to `earnings-script-read`. Route a generic
presenter that is not a meeting recap to `talking-avatar-video`. Route
silent product motion to `product-video-studio`.

Read only the supplied external recap script. Do not add a metric,
quote, forecast, or outcome that is not on the page.
...
- at least one accessible still the host Agent can inspect — a
  presenter portrait or a meeting slide still that will be the first
  frame;
- the post-meeting external script the team already wrote for
  customers;
...
missing hard input. A count outside 2 to 8 is still doable: confirm
that pack size and its live cost.

Do not invent attendance, revenue, pipeline, a quote, or a face. File
...
Write a labeled recap talking list before any paid clone, speech, or
video. Default three slots unless the team names another count in
2 to 8: headline recap, key takeaway, and next step. Each slot records
the still, the spoken line from the external script, intended length
as a 2–15s clip, and whether it uses a catalog voice or a clone.
...
1. Work — one spoken recap line per named slot
...
1. Work — one talking recap clip per named still
...
```text
printf '%s' '{"image":{"type":"artifact","artifact_id":"art-recap-01"},"driving_audio":{"type":"artifact","artifact_id":"art-speech-01"},"prompt":"A restrained customer recap from this still. Keep face, slide, and lighting stable.","duration":8,"client_request_id":"opaque-recap-video-01"}' | python3 scripts/mcp_client.py call beatra.videos.animate
```
...
- For slot lists, payloads, and recovery, read
  [Webinar recap talking workflow](references/workflow.md).
