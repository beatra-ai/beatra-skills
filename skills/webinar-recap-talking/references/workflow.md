# Webinar recap talking workflow

## Slot list

Write one slot per still before any paid call. Default three slots
unless the team names another count in 2 to 8: headline recap, key
takeaway, and next step. Each slot names the still, the spoken line
from the external recap script, and a planned 2–15s window. Do not
invent attendance, revenue, pipeline, a quote, or an outcome that is
not on the script. The labeled list is the free artifact. It is not
approval.
...
Skip clone unless the team wants a cloned voice and can authorize a
...
When the team already supplied an approved speech track for a slot,
...
```json
{
  "input": "Today we covered the roadmap items already written in the external recap script.",
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-recap-speech-01"
}
```

Use `mp3` only when the live speech card supports it and the live
video card accepts `audio/mpeg`. If the team requested `flac`,
...
```json
{
  "image": {"type": "artifact", "artifact_id": "art_recap_01"},
  "driving_audio": {"type": "artifact", "artifact_id": "art_recap_speech_01"},
  "prompt": "A restrained customer recap from this still. Keep face, slide, and lighting stable.",
  "duration": 8,
  "client_request_id": "opaque-recap-video-01"
}
```

Keep `model: "auto"` unless the team chose an eligible video model.
...
the host can actually see and hear. After a returned `task_id`, poll
that task. If the create response is lost, search with
`beatra.tasks.list` and verify with `beatra.tasks.get` before replay.
Reuse an ID only with byte-identical arguments. A changed still, line,
voice, or duration is a new card and a new ID.
