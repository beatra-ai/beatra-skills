...
the script or job-description route, keep compensation, headcount, and
benefits to facts the user supplied. Poll each
...
```json
{
  ...
  "prompt": "A clear hiring delivery with steady eye line, subtle expression, and a stable camera.",
  "duration": 8,
  "client_request_id": "opaque-hiring-role-01-seg-01"
}
```
...
Later roles reuse the frozen `voice_id` and the same portrait artifact
when it still admits. Label the delivered clip with the role name.
