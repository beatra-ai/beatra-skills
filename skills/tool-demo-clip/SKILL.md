---
name: "tool-demo-clip"
description: "Turn authorized tool demo photos and trainer-supplied facts into one tool demo clip per photo. This one-shot demo studio turns each tool photo into a silent 2 to 15 second clip. Use it for enterprise tool demo videos and customer training clips, one photo per clip."
---

# Tool Demo One-Shot Clips

Turn authorized tool demo photos and trainer-supplied facts into one
short clip per photo. Deliver the clips in photo order. Do not stitch
them.

## Scope and adjacent routes

Use this Skill when a training or enablement team wants one short motion
clip for each tool demo photo they already captured.

Route a talking product demo from one photo plus a script to
`product-video-studio`. Route assembly steps to
`assembly-one-step-clip`. Route a generic still that is not a labeled
tool demo to `image-to-motion`. Do not look up public posts for missing
steps.
...
- at least one accessible tool demo photo the host Agent can inspect, in
  capture order;
- trainer-supplied facts that name what each photo shows (tool, action,
  screen, or sequence number).

Do not invent a missing tool step, warning, click path, or setting. A
photo is not a source for unstated instructions. Reuse tool name,
...
2. Write a labeled one-clip plan from the trainer's facts and what the
...
4. Show one current production card and wait before any video
   ...
   1. Work — one tool-demo clip per named photo, a generate call
   ...
   6. If the balance is insufficient — relay the official message and
      ...
      not retry until the trainer says they have topped up.
...
6. Deliver the clips in photo order with actual dimensions, duration,
   usage, and `billing.net_charged_credits`. Review tool identity,
...
reuse a photo outside this demo set.
...
- For clip plans, payloads, confirmation, polling, recovery, and
  review, read [Tool demo one-shot workflow](references/workflow.md).
