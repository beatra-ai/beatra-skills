# Tool demo one-shot workflow

## Build the clip list

Record only trainer-supplied facts. Separate tool identity when given,
each photo in capture order, the tool or action that photo shows,
language, and destination. A missing fact stays a named gap. Do not fill
it from a similar tool manual, a remembered SOP, or what the photo
"probably" shows.
...
```text
python3 scripts/mcp_client.py upload ./tool-demo-01.jpg --mime-type image/jpeg
```
...
```text
printf '%s' '{
  "image": {"type": "artifact", "artifact_id": "art-tool-demo-01"},
  "prompt": "A restrained camera hold on this tool demo frame. Keep the shown tool and action in place. Do not add unstated clicks or warnings.",
  "duration": 5,
  "client_request_id": "opaque-tool-demo-01"
...
Cancel only when the trainer asks, then wait for a terminal state.

Review visible tool identity against the labeled plan. Report unread
