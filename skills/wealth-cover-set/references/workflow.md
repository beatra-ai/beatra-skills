# Wealth cover pack workflow

## Build the pack list

Record only user-confirmed lines. Separate set identity, each named
session and its theme in the user's words, language, destination,
and optional visual references. A missing theme stays a named
gap. Do not fill it from a brand photo, a similar session, or a
remembered product line.

Default plan: one 16:9 2K still per named session, four to eight stills.
If they name fewer than four sessions, keep that count. Cap at eight.
If they want a standard set and have not numbered sessions, use four
roles that already have names. Change canvas only when the user
...
session names and themes. Do not ask the model to invent a session, a
...
```text
printf '%s' '{
  "prompt":"Create the approved session-cover still. Print only the confirmed session name and theme. Keep the shared pack look.",
  "model":"auto",
  "count":1,
  "canvas":{"type":"preset","tier":"2K","aspect":"16:9"},
  "client_request_id":"opaque-wealth-cover-01"
}' | python3 scripts/mcp_client.py call beatra.images.generate
```

A changed session line, language, canvas, model, or prompt is new
work: new card, new ID. Keep at most two generate tasks in flight.

For a local correction after acceptance:

```text
printf '%s' '{"images":[{"type":"artifact","artifact_id":"accepted-cover-still"}],"prompt":"Correct only the approved local change. Keep every other confirmed session line and the shared pack look.","count":1,"client_request_id":"opaque-wealth-cover-edit-01"}' | python3 scripts/mcp_client.py call beatra.images.edit
```
...
type as unread. Do not treat the still as a certified return or a buy
recommendation.
