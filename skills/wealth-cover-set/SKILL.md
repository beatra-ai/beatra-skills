---
name: "wealth-cover-set"
description: "Turn user-supplied session names and themes into a four-to-eight still wealth cover set. This session cover set studio lays out each named investor education cover as its own still. Use it for session cover stills, education session covers, and a matching cover pack."
---

# Wealth Cover Set

Make a four-to-eight still pack from session names and themes the user
already supplied. Use this Skill when a wealth session needs a cover
set — one still per named session — not a risk-grade board and not a
page-by-page factsheet.

Use `risk-grade-set` when the work is one still per named risk grade.
...
detail gallery. Do not look up public posts or market quotes for
missing session titles.

## Collect supplied sessions

Hard inputs are the exact set name and the user-supplied session names
and themes that will appear on the covers. Do not invent a session, buy
...
- exact set, series, or material name;
- each named session and the theme the user already approved;
...
session titles. Uploading makes media available to Beatra and does not
inspect it. When the host cannot view a file, keep the user's
declared role for it.

Default to one still per named session, four to eight stills in the
pack. If they name fewer than four sessions, keep that count. Cap the
pack at eight. If they want a standard set and have not numbered
sessions, plan four stills from the names already given. Omit a
...
read as one wealth cover set.

## Plan the free pack list

Write a labeled pack list before any paid image. For each session
record the printed lines, language, layout (session name, theme,
...
slide and screen covers. Change canvas only when the user names a
...
Read [wealth cover pack workflow](references/workflow.md) when writing
the payload or recovering a task.

## Confirm once, create by session

Planning is free. Before the first billable call, show one current
production card that lists every named session cover, and wait:

1. Work — one session-cover still per named session, a generate call
   (`beatra.images.generate`).
2. Credits — the live `text_to_image` price just read. Do not reuse a
   remembered number.
3. Count — one paid call per session still. This card lists every slot
   in the pack. Do not batch several sessions into one `count`. This
   approval does not cover retries.
4. Identity — one new opaque `client_request_id` per still. A changed
   prompt, session line, file, model, or canvas mints a new ID.
...
Deliver the stills in session order, the pack list, observed dimensions
...
```text
printf '%s' '{"prompt":"Create the approved session-cover still for the named session. Print only the confirmed session name and theme. Keep the shared pack look.","model":"auto","count":1,"canvas":{"type":"preset","tier":"2K","aspect":"16:9"},"client_request_id":"opaque-wealth-cover-01"}' | python3 scripts/mcp_client.py call beatra.images.generate
```
...
- For pack lists, payloads, confirmation, polling, recovery, and review,
  read [wealth cover pack workflow](references/workflow.md).
