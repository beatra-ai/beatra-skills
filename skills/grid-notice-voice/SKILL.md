---
name: "grid-notice-voice"
description: "Turn a written grid-matter list into one grid notice voice clip per labeled cue. This grid matter voice studio records each community grid notice audio and street grid voice from the list the office already wrote, then delivers 8 to 20 grid notice voice pack files. Use it for grid notice clip packs that keep one matter on each clip."
---

# Grid Notice Voice Pack

Turn a written grid-matter list into one spoken clip per labeled
cue. Deliver 8 to 20 reads. Cover time, place, and the matter first.

## Scope and adjacent routes

Use this Skill when a street office needs labeled grid-notice clips
from the matter list they already wrote.

Route generic narration to `voiceover-narration-studio`. Route a
civil-affairs materials list to `civil-affairs-voice`. Route an
elder-checkup schedule to `elder-checkup-voice`. Route a holiday
homework list to `holiday-homework-voice`.

Read only the supplied grid-matter list. Do not invent a notice, an
approval, or an enforcement result.

## Collect the grid-matter list

Hard inputs are:

- the grid-matter list the office already wrote;
...
Do not invent a notice, an approval, or an enforcement result. File
...
Write a labeled grid-notice voice list before any paid clone or
speech. Default ten slots unless the office names another count in
8 to 20: time, place, matter, audience, contact, window, materials,
deadline, reminder, and follow-up. Each slot records the spoken line
from the written grid-matter list and whether it uses a catalog voice
or a clone.
...
1. Work — one spoken grid-matter cue per named slot
   (`beatra.speech.synthesize`).
...
pronunciation table and the written grid-matter list. Report only
what the host can actually hear. Do not invent a notice or an
enforcement result.
...
```text
printf '%s' '{"input":"<the written line for this slot>","voice":"voice_...","format":"mp3","client_request_id":"opaque-grid-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```
