# Grid notice voice workflow

## Slot list

Write one slot per written grid-matter item before any paid call.
Default ten slots unless the office names another count in 8 to 20:
time, place, matter, audience, contact, window, materials, deadline,
reminder, and follow-up. Each slot names the spoken line from the
written grid-matter list. Do not invent a notice, an approval, or an
enforcement result. The labeled list is the free artifact. It is not
approval.
...
Skip clone unless the office wants a cloned staff voice and can
...
```json
{
  ...
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-grid-speech-01"
}
```
...
pronunciation table and the written grid-matter list. Do not invent a
notice or an enforcement result. After a returned `task_id`, poll
