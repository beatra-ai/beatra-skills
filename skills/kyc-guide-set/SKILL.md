---
name: "kyc-guide-set"
description: "Turn user-supplied account-opening material checklist lines into a four-to-eight still KYC guide set. This checklist-to-guide studio lays out each named material as its own still. Use it for account-opening material graphics, KYC guide stills, materials checklist cards, and a KYC materials pack."
---

# KYC Materials Guide Pack

Make a four-to-eight still pack from a materials checklist the user
already supplied. Use this Skill when an account-opening session needs
a matching KYC guide pack — one still per named material — not a
risk-grade pack and not a branch-window board pack.

Use `broker-account-voice` when the work is account-opening voice
steps. Use `risk-grade-set` when the work is one still per named risk
grade. Use `bank-desk-board-set` when the work is one still per named
window. Do not look up public posts for missing material lines. Do
not invent a requirement, rate, ranking, official stamp, or legal
opinion to finish a slot.

## Collect supplied materials

Hard inputs are the exact checklist name and the user-supplied material
names and guide lines that will appear on the stills. Do not invent a
material, rate, hour, ranking, or official stamp to finish a slot.
Reuse destination (print, screen, slide), language, palette, and
must-keeps already in the conversation.

Ask only for facts that change the pack:

- exact checklist, session, or pack name;
- each named material and the guide lines the user already approved;
...
material titles. Uploading makes media available to Beatra and does
not inspect it. When the host cannot view a file, keep the user's
declared role for it.

Default to one still per named material, four to eight stills in
the pack. If they name fewer than four materials, keep that count.
Cap the pack at eight. If they want a standard set and have not
numbered materials, plan four stills from the names already given.
...
read as one KYC materials guide pack.

## Plan the free pack list

Write a labeled pack list before any paid image. For each material
record the printed lines, language, layout (material name, guide
points, optional footer), canvas, and any optional reference role.
...
print handouts. Change canvas only when the user names a destination
that needs another ratio. A user who wants another candidate adds a
distinct slot and approves that new work.
...
Read [KYC materials guide pack workflow](references/workflow.md) when
writing the payload or recovering a task.

## Confirm once, create by material

Planning is free. Before the first billable call, show one current
production card that lists every named material still, and wait:

1. Work — one material still per named item, a generate call
   (`beatra.images.generate`).
2. Credits — the live `text_to_image` price just read. Do not reuse a
   remembered number.
3. Count — one paid call per material still. This card lists every
   slot in the pack. Do not batch several materials into one
   `count`. This approval does not cover retries.
4. Identity — one new opaque `client_request_id` per still. A changed
   prompt, guide line, file, model, or canvas mints a new ID.
...
item, not as a certified legal opinion or an official requirement.

Deliver the stills in checklist order, the pack list, observed
...
```text
printf '%s' '{"prompt":"Create the approved KYC material still for the named item. Print only the confirmed material name and guide lines. Keep the shared pack look.","model":"auto","count":1,"canvas":{"type":"preset","tier":"2K","aspect":"3:4"},"client_request_id":"opaque-kyc-guide-01"}' | python3 scripts/mcp_client.py call beatra.images.generate
```
...
- For pack lists, payloads, confirmation, polling, recovery, and review,
  read [KYC materials guide pack workflow](references/workflow.md).
