# Wrong-item-script talking-clip workflow

## Slot list

Write one slot per still before any paid call. Default three slots
unless the teacher names another count in 2 to 8: mistake cause,
correct path, and common-error reminder. Each slot names the still,
the spoken line from the supplied wrong-item script, and a planned
2–15s window. Do not invent a score, grade, ranking, pass mark, or
unstated answer. The labeled list is the free artifact. It is not
approval.
...
```json
{
  "input": "This clip uses the mistake cause already written on the supplied wrong-item script.",
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-item-speech-01"
}
```
...
```json
{
  "image": {"type": "artifact", "artifact_id": "art_item_01"},
  "driving_audio": {"type": "artifact", "artifact_id": "art_item_speech_01"},
  "prompt": "A restrained wrong-item-script read from this still. Keep face, item graphic, and lighting stable.",
  "duration": 8,
  "client_request_id": "opaque-item-video-01"
}
```
...
points match the supplied wrong-item script. Report only what the
