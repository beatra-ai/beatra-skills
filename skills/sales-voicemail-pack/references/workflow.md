# Sales voicemail workflow

## Slot list

Write one slot per written voicemail before any paid call. Default
ten slots unless the seller names another count in 8 to 20:
missed-call callback, after-hours greeting, follow-up, busy-line,
out-of-area, next-step, reminder, no-answer, thanks, and closing.
Each slot names the spoken line from the written voicemail copy. Do
not invent a phone number, a callback time, a price, or a reading.
...
```json
{
  ...
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-voicemail-speech-01"
}
```
...
pronunciation table and the written voicemail list. After a returned
