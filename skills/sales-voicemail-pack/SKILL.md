---
name: "sales-voicemail-pack"
description: "Turn seller-supplied sales voicemail scripts into one spoken voicemail clip per labeled slot. This sales voicemail studio records each missed-call callback, after-hours greeting, and follow-up voicemail from the lines the seller already wrote. Use it for voicemail drops, sales voicemail packs, and callback voicemail reads."
---

# Sales Voicemail Pack

Turn already-written sales voicemail scripts into one spoken clip
per labeled slot. Deliver 8 to 20 reads. Cover missed-call callback,
after-hours greeting, and follow-up first.

## Scope and adjacent routes

Use this Skill when a seller needs labeled voicemail clips they can
import into a phone or dialer.
...
Read only the supplied voicemail copy. Do not add a phone number,
callback time, price, or reading that is not on the list.

## Collect the voicemail lines

Hard inputs are:

- the missed-call, after-hours, and follow-up lines the seller
  already wrote;
...
Do not invent a phone number, a callback time, a price, or a
reading. File access is not consent.
...
Write a labeled voicemail list before any paid clone or speech.
Default ten slots unless the seller names another count in 8 to 20:
missed-call callback, after-hours greeting, follow-up, busy-line,
out-of-area, next-step, reminder, no-answer, thanks, and closing.
Each slot records the spoken line from the written voicemail copy
...
1. Work — one spoken voicemail per named slot
   (`beatra.speech.synthesize`).
...
pronunciation table and the written voicemail list. Report only what
the host can actually hear.
...
```text
printf '%s' '{"input":"<the written line for this slot>","voice":"voice_...","format":"mp3","client_request_id":"opaque-voicemail-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```
