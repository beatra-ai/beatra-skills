# Nutrition-panel workflow

## Build the fact list

Record only seller-confirmed lines. Separate SKU identity, printed
nutrition text, serving size, language, destination, and optional
visual references. A missing line stays a named gap. Do not fill it
from a pack photo, a similar SKU, or a remembered regulation table.

Default plan: one square 2K still per SKU. Change canvas only when the
seller names a destination that needs another ratio. Default four
SKUs unless the seller names another count in 4 to 8.
...
lines. Do not ask the model to infer calories or daily values.
...
```text
printf '%s' '{
  "prompt":"Create the approved nutrition panel. Print only the confirmed SKU name and nutrition lines. Do not add extra nutrient numbers.",
  "model":"auto",
  "count":1,
  "canvas":{"type":"preset","tier":"2K","aspect":"1:1"},
  "client_request_id":"opaque-nutrition-sku-01"
...
```text
printf '%s' '{"images":[{"type":"artifact","artifact_id":"accepted-card"}],"prompt":"Correct only the approved local change. Keep every other confirmed line.","count":1,"client_request_id":"opaque-nutrition-edit-01"}' | python3 scripts/mcp_client.py call beatra.images.edit
```
...
type as unread. Do not treat the still as a certified legal nutrition
label.
