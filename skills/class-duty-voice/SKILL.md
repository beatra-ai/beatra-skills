---
name: "class-duty-voice"
description: "Turn a written class duty roster into one class duty voice clip per labeled cue. This duty roster voice studio records each class duty audio and roster reminder from the list the teacher already wrote, then delivers 8 to 20 class duty voice pack files. Use it for duty clip packs that keep one roster item on each clip."
---

# Class Duty Voice Pack

Turn a written class duty roster into one spoken clip per labeled cue.
Deliver 8 to 20 reads. Cover weekday names and the listed chores first.

## Scope and adjacent routes

Use this Skill when a teacher needs labeled class-duty reminder clips
from the roster they already wrote.

Route generic narration to `voiceover-narration-studio`. Route a
homeroom weekly list to `homeroom-week-voice`. Route holiday homework
to `holiday-homework-voice`. Route a census schedule to
`census-notice-voice`.

Read only the supplied duty roster. Do not invent who skipped duty, a
punishment, a grade, or a missing name.

## Collect the roster

Hard inputs are:

- the class duty roster the teacher already wrote;
...
Do not invent who skipped duty, a punishment, a grade, or a missing
name. File access is not consent.
...
Write a labeled class-duty voice list before any paid clone or
speech. Default ten slots unless the teacher names another count in
8 to 20: Monday, Tuesday, Wednesday, Thursday, Friday, weekend,
broom, chalkboard, watering, and extra. Each slot records the spoken
line from the written roster and whether it uses a catalog voice or a
clone. Omit a default role whose line is missing on the roster.
...
sentence boundaries. Write short spoken sentences from the roster.
...
If the teacher wants a cloned staff voice, inspect an authorized
...
1. Work — one spoken roster cue per named slot
   (`beatra.speech.synthesize`).
...
pronunciation table and the written duty roster. Report only
what the host can actually hear. Do not invent who skipped duty.
...
```text
printf '%s' '{"input":"<the written line for this slot>","voice":"voice_...","format":"mp3","client_request_id":"opaque-duty-speech-01"}' | python3 scripts/mcp_client.py call beatra.speech.synthesize
```
