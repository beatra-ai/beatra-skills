# Class duty voice workflow

## Slot list

Write one slot per written roster item before any paid call.
Default ten slots unless the teacher names another count in 8 to 20:
Monday, Tuesday, Wednesday, Thursday, Friday, weekend, broom,
chalkboard, watering, and extra. Each slot names the spoken line from
the written class duty roster. Omit a default role whose line is
missing. Do not invent who skipped duty, a punishment, a grade, or a
missing name. The labeled list is the free artifact. It is not
approval.
...
Skip clone unless the teacher wants a cloned staff voice and can
...
```json
{
  ...
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-duty-speech-01"
}
```
...
pronunciation table and the written duty roster. Do not invent who
skipped duty. After a returned `task_id`, poll that task. If the
