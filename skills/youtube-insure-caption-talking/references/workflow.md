# Insurance caption talking-clip workflow

## Slot list

Write one slot per chosen caption point before any paid call.
Default three slots unless the advisor names another count in 2 to
8. Each slot names the original caption wording, the speakable line
from the supplied clause draft, the still, and a planned 2–15s
window. Do not invent a caption line, coverage term, deductible,
waiting period, claim result, or payout. The labeled list is the
free artifact. It is not approval.

If the advisor wants this connection to read public YouTube
insurance captions, follow [caption lookup](caption-lookup.md) on
its own card first.
