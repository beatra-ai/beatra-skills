...
Use this text-only postpaid task before generation when multimodal context or a
rough brief needs a production-ready prompt:
...
```json
{
  ...
  "duration": 5,
  "aspect_ratio": "16:9",
  "client_request_id": "opaque-enhance-prompt-id"
}
```
...
Use when the ending image must be exact. Add `first_frame` when both boundaries
matter:
...
`last_frame` is required; `first_frame` is optional. When both are supplied,
admit their relationship and any optional driving audio, then check ratio and
composition compatibility before spend. The inputs define endpoints, not a
guarantee about every generated intermediate frame.
