# Dividend talking-clip workflow

## Slot list

Write one slot per still before any paid call. Default three slots
unless the advisor names another count in 2 to 8: dividend notice,
record date, and payment arrival. Each slot names the still, the
spoken line from the supplied dividend announcement, and a planned
2–15s window. Do not invent a dividend amount, ex-date, payment date,
yield, forecast, buy recommendation, or account credit not on the
announcement. The labeled list is the free artifact. It is not
approval.
...
```json
{
  "input": "The per-unit dividend amount is the one already printed on this announcement.",
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-div-speech-01"
}
```
...
```json
{
  "image": {"type": "artifact", "artifact_id": "art_div_01"},
  "driving_audio": {"type": "artifact", "artifact_id": "art_div_speech_01"},
  "prompt": "A restrained dividend announcement read from this still. Keep face, announcement graphic, and lighting stable.",
  "duration": 8,
  "client_request_id": "opaque-div-video-01"
}
```
...
lines match the supplied dividend announcement. Report only what the
