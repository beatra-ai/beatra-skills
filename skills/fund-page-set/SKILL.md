---
name: "fund-page-set"
description: "Turn user-supplied fund factsheet points into one still per page. This fund factsheet page studio lays out each prospectus page still from the supplied fund factsheet. Use it for factsheet page graphics, fund page set layouts, and page-by-page fund stills."
---

# Fund Factsheet Pages

Make one prospectus still per named page or step from facts the user
already supplied. Use this Skill when a wealth session needs a
page-by-page fund still set, not a four-to-eight product gallery and
...
`amazon-a-plus-module-pack` when that package is installed. Do not look
up public posts or market quotes for missing factsheet lines.

## Collect supplied pages

Hard inputs are the exact fund name and the user-supplied factsheet
page or step points that will appear on the stills. Do not invent a
NAV, return, ranking, buy recommendation, risk grade, or fee line to
finish a page. Reuse destination (handout, screen, print), language,
...
- the factsheet lines the user has already approved for each page;
...
numbers. Uploading makes media available to Beatra and does not
...
several stills, each with its own fact list. Omit a line whose fact
is missing and keep that gap on the plan so the user can add it
later.
...
Read [fund-page workflow](references/workflow.md) when writing the
...
1. Work — one prospectus still per named page or step, a generate
...
item, not as a certified disclosure.
...
```text
printf '%s' '{"prompt":"Create the approved fund factsheet page still for the named page. Print only the confirmed factsheet lines.","model":"auto","count":1,"canvas":{"type":"preset","tier":"2K","aspect":"3:4"},"client_request_id":"opaque-fund-page-01"}' | python3 scripts/mcp_client.py call beatra.images.generate
```
...
- For page lists, payloads, confirmation, polling, recovery, and review,
  read [fund-page workflow](references/workflow.md).
