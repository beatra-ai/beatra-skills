# Restock talking-clip workflow

## Clip list

Write one list item per still before any paid call. Default three
clips unless the seller names another count in 2 to 8: what is back,
when it lands, and next step. Each item names the still, the spoken
line from confirmed restock facts, and a planned 2–15s window. Do not
invent a restock date, price, unit count, or unseen SKU. The labeled
...
```json
{
  "input": "<the written restock line for this clip>",
  "voice": "voice_...",
  "format": "mp3",
  "client_request_id": "opaque-restock-speech-01"
}
```
...
```json
{
  "image": {"type": "artifact", "artifact_id": "art_restock_01"},
  "driving_audio": {"type": "artifact", "artifact_id": "art_restock_speech_01"},
  "prompt": "A restrained restock announcement from this still. Keep face, product, and lighting stable.",
  "duration": 8,
  "client_request_id": "opaque-restock-video-01"
}
```
